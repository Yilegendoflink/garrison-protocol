...
## 七、续修：波登可的瓶子与「通用兜底圈／兜底状态」误伤（用户 2026-09-22 第二次报障）

用户报「波登可的技能投掷瓶子的效果不正确，和 PRTS 不一致」。核对后确认是**通用兜底分支**的问题，不是数据问题：

* PRTS 技能备注：**「※孢子群范围半径为0.9，可对空」**；描述：碎裂后在周围产生持续 5 秒（专三 6 秒）的孢子群，
  范围内所有敌人**被停顿且失去特殊能力**，每秒受到攻击力 40%（专三 80%）的法术伤害。
* 修前：`native-operator-effects.operatorSkillStart` 里「文案出现 每秒＋受到/造成＋伤害 就建圈」的兜底分支
  在**波登可自己脚下**又建了一个 5×5（radius 2）的伤害圈，而真正的投掷圈由 `native-effects` 的 `podego-s2` 生成——
  于是瓶子落点 + 自己脚下各有一份伤害，且「失去特殊能力」还会被通用「开技时对攻击范围内所有敌人施加状态」的兜底
  漏给整个攻击范围（相邻格敌人也中招）。
* 修法：
  1. `podego-s2` 半径 1 → **0.9**（并标 `shape:'circle'`），落地即施加停顿＋失去特殊能力，每秒 1 次伤害、持续 5/6 秒、可对空；
  2. 通用周期伤害圈改成**白名单** `GENERIC_ZONE_SKILLS`（只剩浊心斯卡蒂 S3、深靛 S2），且文案写「范围内」时挂 `rangeUid`；
  3. 通用「开技即施加状态」加 `DEDICATED_STATUS_SKILLS`（波登可 S2、异客 S3、寒檀 S2——状态归它们的专属圈管）；
  4. 地灵 S2「流沙化」补专属实现：范围内每 1.6 秒（按等级）停顿 0.8 秒、**没有伤害**、技能期间缴械（此前只有兜底伤害圈，
     移除兜底后会变成空技能）；`applyStatus(...,'sluggish')` 的时长支持 `values.sluggishTime`。

被这轮清掉的兜底圈（它们的真实伤害都在别处，逐个核对过，没有功能回退）：
塞雷娅 S3（专属圈）、莫斯提马 S2（专属圈）、歌蕾蒂娅 S3（专属圈）、菲莱 S2（受击反伤）、蕾缪安 S3（技能结束轰炸）、
焰影苇草 S2/S3（火球与灼痕目标各自结算）、烛煌 S2/S3（灼烧地段与自身流失）、荒芜拉普兰德 S3（浮游单元周围减速）、
新约能天使 S2（每次攻击）、溯光星源 S3（链接传导）、地灵 S2（改为专属停顿圈）。
