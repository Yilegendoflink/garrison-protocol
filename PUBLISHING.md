...
当前可试玩版本不满足此条件。`readiness.json` 的 `s6.playableParityComplete` 为 false，S04／S05 也存在未完成项。不得只修改标志来绕过用户要求。
...
1. 关闭 S04–S06 所有影响玩法／操作的缺口。正式入口必须运行原作模式，并通过全部内容、难度、关键组合和原作对照验收；资料归档数量不作为完成依据。
