...
- `native-waves.js` / `native-wave-random.js` / `native-wave-editor.js`：词条预算抽怪与编制台（编制台有两个页面：敌人编制／盟约禁用）
- `native-bond-ban.js`：盟约禁用（每局 3 核心 + 4 附加、逐盟约的「不禁用名单」、配置读写与简报摘要）
