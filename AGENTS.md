...
- **通用周期伤害圈（`skill-zone:*`）打谁按「分句」判，不按整段文案**：`native-operator-effects.operatorSkillStart` 那条「每秒受到／造成伤害」的兜底分支里，只有**同一个分句**同时出现「友方单位／友方干员／友军／我方单位」与「受到／造成／伤害／损伤」、且该分句不含「回复／恢复／治疗」时才 `trackSide:'all'`（打友军），否则只打敌人；两侧都没有真正的伤害分句（纯治疗文案）就干脆不建圈。**别退回「整段里出现过友方二字就打友军」**：塞雷娅 S3「钙质化」的文案是「友军每秒回复生命／**敌军**受到的法术伤害提升」两句拼一段（`敌军` 也得算敌方），浊心斯卡蒂 S3「潮涌，潮枯」是同一句里「敌人每秒受到真实伤害」＋「友方单位获得鼓舞」——两例都只该打敌人，整段判定会自己人打自己人；流明 S1「友方每秒受到…**治疗**效果」根本不是伤害，此前还会给治疗技挂一个打敌人的持续伤害圈。`values.type` 认「真实伤害」为 `'true'`。回归：`tests/native-operator-effects.test.mjs`、`tests/native-summon-lifecycle.test.mjs`。
