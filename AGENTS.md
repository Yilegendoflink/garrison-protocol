...
- **死亡留下的持续伤害圈（DOT）口径已定**：`trigger` 为 `death`／`death-target` 的地面区域统一 **100 点 / 0.5 秒、固定伤害**（`native-combat.js` 的 `tuneDeathZone`，落盘在 `enemyBehavior.deathZone`）。施放者随敌人死亡一起消失，按攻击力百分比只能算出 0。`attackZone`（火炮 150/1s、战车 50/1s）与 `selfField`（巢涌者神经损伤）保持原表数值，不要顺手改。**重叠的圈只结算一层**：`NativeBattle.applyEnemyZoneDamage` 用按「排定结算时刻（`fx.nextAt`）」记账的 0.45 秒窗口，弱的不叠加、强的只补差额；窗口戳不能用当前帧时间，否则大步补拍会被压成一拍。数字改动要同时跑 `tests/native-enemy-ground-zone.test.mjs`（含全表门禁：任意新增 death 圈都必须是 100/0.5）。
