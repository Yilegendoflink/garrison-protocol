...
- `native-waves.js` / `native-wave-random.js` / `native-wave-editor.js`：词条预算抽怪与「协议自定义」（三个页面：敌人波次／盟约禁用名单／禁用方案）
- `native-bond-ban.js`：盟约禁用（每局随机 3 核心 + 4 附加、逐盟约三态「固定禁用／参与随机／不被禁」、逐盟约的「不禁用名单」、配置读写与简报摘要）
...
**当前事实：** 本页、`README.md`、`OPERATOR_ADAPTERS.md`、`OPERATOR_COMPLETION_PLAN.md`、`COST_INTERACTIONS_AUDIT.md`、`docs/ENEMY_BEHAVIOR_DEVELOPMENT.md`、`docs/SKILL_RANGE_AUDIT_2026-09-22.md`（技能范围 vs PRTS 的缺口清单）、`PUBLISHING.md`、`COMBAT_RULES.md`。

**盟约禁用（作战前简报）已实现，名单为用户核对过的真实数据：** 机制在 `dist/native-bond-ban.js`，逐盟约影响表见 `docs/BOND_BAN_RESEARCH_TABLES.md`，官方文本证据见 `docs/BOND_BAN_RESEARCH.md`。**每局从「参与随机」的盟约里随机禁 3 个核心 + 4 个附加**（`bondBanIds(data,seed,config)`，与商店/波次随机流分开，同一 seed 结果固定）；**每个盟约的三种状态（固定禁用／参与随机／固定不被禁）在「协议自定义 → 禁用方案」页调**（`banRules()` 解析配置里的 `always`／`never` 两个数组，`banPool()` 把两者都排除出抽取池，**固定禁用的不占那 3＋4 个名额**，对应盟约每局必缺席）；**默认方案把 协防干员（`emptyShip`）／绝技（`suntShip`）／调和（`maniShip`）／独行（`soloShip`）＋投资人（`investShip`）设为「固定不被禁」**（`BOND_BAN_EXCLUDED` 是那四个内置豁免，`BOND_BAN_DEFAULT_NEVER` 再加上投资人；用户 2026-09-22 补充口径），所以默认池是 8 核心 + 10 附加共 18 个，不看 `modeDataDict[].inactiveBondIdList` / `constData.trBannedBondIds`。**判定口径＝每个盟约各有一份「不禁用名单」：被禁盟约的成员只有名单上的还能出场，其余全部禁用——哪怕它还挂着别的没被禁的盟约**；「名下所有盟约都被禁才禁用」是错的第一版，别再退回去。**名单语义（用户 2026-09-22 确认）＝「只有被所有属于它的被禁盟约的名单都写上才保留」**：一名干员属于 2–3 个盟约时，只要有一个被禁盟约的名单上没写它，就出局（例：`空弦` 在拉特兰名单上，灵巧被禁时照样出局；`烛煌` 在维多利亚与炎两份名单上，两个盟约同时被禁也保留）——别再「优化」成「任一被禁盟约名单上有它就放行」（那版已否决，实测中位 44 vs 48）。名单是用户 2026-09-22 逐条给的（`BOND_EXEMPT_TABLE`，按「显示名＋阶」登记，`resolveBondExempt` 解析成 charId，`tests/native-bond-ban.test.mjs` 有门禁：19 条名单一条都不许解析失败、且必须覆盖全部可禁盟约）。改动名单改 `BOND_EXEMPT_TABLE`，或在「协议自定义 → 盟约禁用」页勾选、「禁用方案」页改状态（`localStorage` 键 `garrison-bond-ban-v1`，**带 `version:2`**，旧版本配置会被忽略并回落到内置名单，同版本缺 `always`／`never` 的旧配置按默认方案补齐，可导出 `garrison-bond-ban.json`）。**道具相关的盟约不受影响**（装备自身 `giveBondId` 归属、变形同构体借盟约都照旧），受限的只有**干员的获取**。**战前预览的呈现**（`bondBanBriefingHtml`／`bannedOperatorsHtml`，也在 `native-bond-ban.js` 里、`native-play` 只注入 `esc`／`avatar` 与动作）：**全部核心盟约都要列出**，被禁的灰掉＋名字划掉（`.native-ban-bond.banned`）、没被禁的带 `.available`；**被禁的附加盟约单独一组**同样灰掉划掉（没被禁的附加盟约不列）；被禁干员太多不铺在页面上，改为「查看本局被禁用的 N 名干员」按钮 → 弹窗用头像网格列出（每人标出被哪几个缺席盟约挡下）。改动这两段时同步 `tests/native-bond-ban.test.mjs` 的渲染断言（那边用替身 `esc`／`avatar` 直接断言 HTML）。边界（0 成员盟约已随「固定不参与」一并解决）见 `docs/BOND_BAN_RESEARCH.md` 第五节。
