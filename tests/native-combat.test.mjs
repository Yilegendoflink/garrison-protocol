...
test('native enemy special attacks consume their trigger and reveal invisible attackers',()=>{
 ...
 const hidden={uid:701,atk:100,damageType:'physical',invisible:true,specialSkill:{prefab:'InvisibleCombat'},specialAtkScale:2};let scale=0;b.hurt=(_u,e)=>{scale=e.atk/100;};b.resolveEnemyStrike(hidden,u,{});assert.equal(scale,2);assert.equal(hidden.invisible,false);assert.ok(hidden.invisibleRecoverAt>0);
});
