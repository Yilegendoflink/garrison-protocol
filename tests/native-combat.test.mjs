...
test('enemy partner auras couple guard and blade behavior without leaking to distant allies',()=>{
 const b=liveBattle(),u=b.s.units[0];u.x=0;u.y=0;const guard={uid:800,id:'enemy_1174_duholy',range:1.4,enemyTalent:{'traitAbility.range_radius':1.1,'traitAbility.attack_speed':-30},name:'深池伙友卫队',x:0,y:0,hp:100,baseDef:0,baseRes:0,def:0,res:0};const blade={uid:801,id:'enemy_1175_dushdo_2',range:1.4,enemyTalent:{'traitAbility.base_attack_time':-1.3},name:'深池伙友影刃精英',x:1,y:0,hp:100,baseDef:0,baseRes:0,def:0,res:0};b.s.enemies=[guard,blade];b.refreshEnemyAuras();assert.equal(u.enemyAttackSpeedMod,-30);assert.equal(blade.attackIntervalMod,-1.3);const distant={...u,uid:999,x:5,y:5,deployed:true,hp:100};b.s.units.push(distant);b.refreshEnemyAuras();assert.equal(distant.enemyAttackSpeedMod,0);
});
