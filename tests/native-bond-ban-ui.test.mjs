...
// 盟约禁用的接线（用户 2026-09-22 二次修订口径：所属盟约全被禁才禁用、配置只有三态）：机制与判定在 dist/native-bond-ban.js，
...
test('协议自定义只有两页：敌人波次 + 禁用方案（固定禁用／随机／不被禁）',()=>{
 // v3 起「盟约禁用名单」页整页删除：编制台只控制各盟约是否随机被禁或固定被禁。
 assert.doesNotMatch(editor,/data-page="bonds"/,'名单页入口要删掉');
 assert.doesNotMatch(editor,/renderBondBanPage/,'名单页渲染入口要删掉');
 assert.doesNotMatch(editor,/ed-bb-toggle|ed-bb-all|ed-bb-defaults|ed-bb-clear|bondQuery/,'名单页的勾选／整盟约开关／恢复内置名单动作一并删掉');
 assert.doesNotMatch(editor,/exempt/,'编制台不再有逐盟约的不禁用名单');
 ...
 assert.match(editor,/saveBondBan\(/,'每次改动都要落盘（localStorage）');
 ...
 assert.match(editor,/const title=page==='rules'\?'盟约禁用方案':'协议自定义'/,'页面标题随 tab 切换');
...
test('默认不被随机禁用的盟约（含投资人）与三态配置都在界面上说清楚',()=>{
 ...
 assert.match(banModule,/row\.bonds\.every\(bond=>set\.has\(bond\)\)/,'干员禁用判定＝所属盟约全部被禁才禁用');
 assert.match(editor,/BAN_MODE_LABEL=\{fixed:'固定禁用',random:'随机候选',never:'固定不被禁'\}/,'三种状态要有人话标签');
 assert.match(editor,/恢复默认方案/,'「恢复默认」恢复的是默认三态方案');
...
test('战前预览：核心盟约全列＋被禁的灰掉划掉，附加盟约单列，被禁干员进弹窗',()=>{
 ...
 assert.match(css,/\.native-ban-group\{[^}]*border-top/,'按被禁盟约分组的样式（组与组之间要能分开）');
 assert.match(banModule,/data-bond="\$\{esc\(b\.id\)\}"/,'弹窗要按被禁盟约分组渲染');
 assert.match(banModule,/同一名干员挂在多个缺席盟约下时，会在每组各列一次/,'重复列出的口径要写清楚');
});
