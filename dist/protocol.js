...
// 整备资金（用户 2026-09-22 口径）：开局 4，每回合默认多获取 1，**默认收入封顶 11**。
// 上限只约束「默认回合收入」这一条路径（下面 baseFunding → nextRound 的 setFunds）；
// 效果／策略／装备发的额外资金一律走 addFunds，不受它限制；海猫模式仍是 INFINITE_FUNDS。
...
// ── 战斗中计数器（回合 / 击杀·当轮敌人总数 / 剩余生命值）──────────────────
// 击杀数**不含衍生敌人**：解压缩碎片、敌方召唤／分裂／幻影这些战斗中途生成的敌人不是本波编制，
// 单独记在 `s.derivedKills` 里（`s.kills` 保持原语义，战报记录与卫戍击倒计数都不受影响）。
// 分母是**当轮敌人总数** `s.total`（编波时按 `notCountInTotal` 过滤后的数量），不是剩余数。
export function battleTally(s){
 return {kills:Math.max(0,(Number(s?.kills)||0)-(Number(s?.derivedKills)||0)),derived:Number(s?.derivedKills)||0,total:Number(s?.total)||0};
}
...
export class PreparationState {
 ...
 upgrade(){const price=this.terms().upgradeCost;if(this.s.phase!=='prep'||price===null||this.s.funds<price||this.s.rewardPending)return false;this.addFunds(-price);this.s.level++;this.s.discount=0;return true;} // discount 清零：升到新等级后按该级的 initialUpgradePrice 原价重新开始每回合 -1
