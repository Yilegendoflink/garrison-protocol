...
const ACTIONS={
 ...
 deploy:{phase:'prep',method:'deploy',check:a=>a.length===3&&a.every(INTEGER)},turn:{phase:'prep',method:'turn',check:a=>a.length===2&&INTEGER(a[0])&&INTEGER(a[1])&&a[1]<4},
...
export class GameSession {
 ...
 preview(type,args=[]){const r=this.inspect(type,args);return {ok:r.ok,code:r.code,message:r.message||null,phase:this.game.s.phase,revision:this.revision};}
 dispatch(command){
...
export class InteractionState {
 ...
 select(uid,phase){this.uid=uid;this.phase=phase;this.mode='selected';this.cell=null;}
 preview(session,x,y){if(this.uid===null)return {ok:false,code:'NO_SELECTION'};const result=session.preview('deploy',[this.uid,x,y]);this.cell=result.ok?{x,y}:null;this.mode='deploy-preview';this.revision=session.revision;this.phase=session.game.s.phase;return result;}
 commit(session){if(!this.cell||this.phase!==session.game.s.phase||this.revision!==session.revision){this.cancel();return {ok:false,code:'STALE_PREVIEW'};}const r=session.send('deploy',this.uid,this.cell.x,this.cell.y);if(r.ok)this.cancel();return r;}
 cancel(){this.mode='idle';this.uid=null;this.cell=null;this.phase=null;this.revision=null;}
}
