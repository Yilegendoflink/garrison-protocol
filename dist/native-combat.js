...
export function enemyBehaviorProfile(raw={}){
 ...
 if(/^enemy_1067_snslime(?:_2)?$/.test(raw.prefabKey||''))explosion=Number(bb['boom.atk_scale'])>0&&Number(bb['boom.freeze'])>0?{type:'physical',scale:Number(bb['boom.atk_scale']),cold:Number(bb['boom.freeze']),radius:1.65,delay:1,groundOnly:true,silenceable:true}:null;
 ...
 return {
  ...
  staticRigid:behavior.staticRigid===true,nonPrimary:behavior.nonPrimary===true,notCountInTotal:behavior.notCountInTotal??raw.notCountInTotal??false,isolated:behavior.isolated===true,
...
export function advanceEnemy(e,dt,onEvent,stopForAttack=false){
 ...
 while(e.cmd<e.route.length){
  ...
  const speed=(!e.block&&permissions(e).move&&!stopForAttack)?enemyMovementSpeed(e)*(e.moveSpeedMod??1)*(e.waterMoveScale??1)*(e.sandMoveScale??1)*(e.envMoveScale??1)*slow:0;
