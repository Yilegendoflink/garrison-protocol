...
function elementBurstActive(target,type,now){return !!target&&(Number(target.elementBurstUntil)||0)>now&&target.elementBurstType===type;}
...
// 通用「天赋附带元素损伤」通道要跳过的干员：他们要么已有专属实现、要么口径不是「每次攻击」（塑心是每秒、苇草是概率触发的灼痕）。
const TALENT_ELEMENT_DEDICATED=new Set(['char_245_cello','char_1020_reed2']);
...
const DEDICATED_STATUS_SKILLS=new Set([ 'char_258_podego#1', // 孢子扩散：状态在 podego-s2（落点孢子群）
 'char_472_pasngr#2', // 辉煌裂片：状态在 pasngr-s3（雷暴区域）
 'char_341_sntlla#1'  // 女巫之泪：状态在 sntlla-s2（冰凌落点）
]);
// 通用「攻击附带元素损伤」兜底的白名单（当前为空）：进表前必须核对 PRTS 文案确认元素是**这次攻击**造成的，
// 并且本文件里没有专属实现（见 after-damage 段的说明与 docs/ELEMENT_DAMAGE_AUDIT_2026-09-22.md）。
const GENERIC_ELEMENT_SKILLS=new Set([]);
const NUMERIC_KEYS={
...
// 「周围/攻击范围内的地面敌人」统一口径：优先按技能 rangeId 的网格（随朝向旋转），
// 没有 rangeId 时退回以自身为中心的切比雪夫方格（radius 默认 1）。
function zoneTargets(battle,source,rangeId,radius=1){
 const grids=rangeId?battle?.data?.ranges?.[rangeId]?.grids:null;
...
export function operatorSkillStart(battle,u,ctx){
 ...
 if(profile.charId==='char_4010_etlchi'&&skillIndex===1){const anchor=allAllies(battle,u,true).filter(a=>a.uid!==u.uid&&battle.profile(a).position==='MELEE')[0]||null;const anchors=[u,...(anchor&&anchor.uid!==u.uid?[anchor]:[])];for(const at of anchors)ctx.addEffect(battle,{kind:'zone',sourceUid:u.uid,sourceDeployGen:u.deployGen,talentOrSkillId:'etlchi-s1-'+at.uid,x:at.x,y:at.y,radius:1,interval:Number(bb.interval)||.5,nextAt:battle.s.time+.5,endsAt:battle.s.time+(profile.skill.duration>0?profile.skill.duration:10),trackArea:true,trackSide:'enemy',values:{dot:true,type:'physical',atk_scale:Number(bb.atkScale)||1.35},snapshot:{damage:battle.stats(u).atk*(Number(bb.atkScale)||1.35)},refKind:'owner',persistAfterSourceGone:false});return true;}
 ...
 if(profile.charId==='char_311_mudrok'&&skillIndex===1){u.mudrokS1={skillCount:u.skillCount,scale:Number(bb.atkScale)||1.9,radius:1,rangeId:profile.skill?.rangeId||null,stun:Number(bb.stun)||.5,heal:Number(bb.hp_ratio)||.04};suppressDefault=true;}
 ...
 // 余 S3「灶里乾坤」的火墙（PRTS 备注）：**垂直于自身部署朝向、位于自身部署方向前方 0.5 格、长度整个战场**。
 // 它本身不造成伤害：① 分处火墙两侧的「友方 → 敌方」**法术伤害**附带余攻击力 ep_damage_ratio 的灼燃损伤
 // （损伤来源为余；余自身造成的伤害与无来源伤害不触发，见 native-effects.firewallElementBurn）；
 // ② 敌方**可消除弹道**（速度 0<v≤9）穿过火墙时按 `prob` 概率被清除（见 native-enemy-attacks）。
 if(profile.charId==='char_2026_yu'&&skillIndex===2){const d=directionOf(u.dir||0),axis=d[0]!==0?'x':'y',at=(axis==='x'?u.x:u.y)+.5*(axis==='x'?d[0]:d[1]);ctx.addEffect(battle,{kind:'zone',sourceUid:u.uid,sourceDeployGen:u.deployGen,talentOrSkillId:'yu-firewall',x:u.x,y:u.y,radius:0,interval:1,nextAt:battle.s.time+1,endsAt:profile.skill.duration>0?battle.s.time+profile.skill.duration:null,trackArea:false,values:{firewall:{axis,at,prob:Number(bb.prob)||.1,elementRatio:Number(bb.ep_damage_ratio)||.06}},snapshot:{}});}
 if(profile.charId==='char_4116_blkkgt'&&skillIndex===2){const count=10,interval=Number(bb.d_hit_interval)||.3;ctx.addEffect(battle,{kind:'zone',sourceUid:u.uid,sourceDeployGen:u.deployGen,talentOrSkillId:'blkkgt-s3',x:u.x,y:u.y,radius:2,interval,nextAt:battle.s.time+interval,endsAt:battle.s.time+count*interval,trackArea:true,trackSide:'enemy',values:{dot:true,pull:true,type:'physical',shape:'circle',atk_scale:Number(bb.d_atk_scale)||1.8},snapshot:{damage:battle.stats(u).atk*(Number(bb.d_atk_scale)||1.8)},refKind:'owner',persistAfterSourceGone:false});return true;}
 if(profile.charId==='char_4116_blkkgt'&&skillIndex===0){u.blkkgtNext={skillCount:u.skillCount,scale:Number(bb['atk_scale_s1'])||1.75,hits:2,max:Number(bb.max_target)||1,rangeId:profile.skill?.rangeId||null};suppressDefault=true;}
 ...
 // 凛御银灰技能三只处理费用与待部署区费用互换；「风雪之眼」本身是待部署区卡牌（她技能三期间才可部署），
 // 按用户口径（2026-09-19）本期不实现，不生成任何场上实体。
 ...
 // 纯烬 S2「云霭荫佑」：治疗一次 + 生成**损伤屏障**（吸收元素损伤，不是普通护盾）。
 // PRTS：范围内友方**共享**吸收值、可对后续部署于范围内的目标生效、吸收值以释放技能时的攻击力为准；重复释放重置。
 if(profile.charId==='char_1016_agoat2'&&skillIndex===1){for(const a of allAllies(battle,u,true))ctx.applyHeal(battle,{source:u,target:a,amount:battle.stats(u).atk});
  const amount=battle.stats(u).atk*(Number(bb['agoat2_s_2[shield].atk_scale'])||5);battle.s.elementBarriers=(battle.s.elementBarriers||[]).filter(b=>b.refUid!==u.uid);
  battle.s.elementBarriers.push({refUid:u.uid,sourceUid:u.uid,amount,remaining:amount,until:battle.s.time+(Number(bb.duration)||12)});return true;}
 // 纯烬 S1「无声润物」：攻击范围内友方**每秒**回复攻击力 4% 的元素损伤（元素治疗，无视禁疗）。
 // 旧实现用 1/30 秒的结算间隔却给整秒的量，实际是 30 倍——这里改成与文案一致的「每秒」。
 if(profile.charId==='char_1016_agoat2'&&skillIndex===0){ctx.addEffect(battle,{kind:'zone',sourceUid:u.uid,sourceDeployGen:u.deployGen,talentOrSkillId:'agoat2-s1',rangeUid:u.uid,x:u.x,y:u.y,radius:1,interval:1,nextAt:battle.s.time+1,endsAt:null,trackArea:true,trackSide:'ally',values:{elementRegen:battle.stats(u).atk*(Number(bb['agoat2_s_1[aura].ep_heal_ratio'])||.04),elementRegenIgnoresHealBlock:true},snapshot:{}});}
 // 纯烬 S3「火山回响」：治疗与**元素损伤回复**的 5 连发，优先治疗不同的目标；
 // PRTS 索敌：本次动作内没选过的 > 生命值百分比最低 > 元素值最低 > 更早部署；只选生命值或元素值未满的单位。
 if(profile.charId==='char_1016_agoat2'&&skillIndex===2){const scale=Number(bb['attack@heal_scale'])||.35,chosen=[];for(let n=0;n<5;n++){const pool=allAllies(battle,u,true).filter(a=>a.hp<a.maxHp||battle.elementInjury?.(a)>0);const fresh=pool.filter(a=>!chosen.includes(a.uid)),pick=(fresh.length?fresh:pool).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp||battle.elementInjury?.(b)-battle.elementInjury?.(a)||(a.deployAt??0)-(b.deployAt??0)||a.uid-b.uid)[0];if(!pick)break;chosen.push(pick.uid);ctx.applyHeal(battle,{source:u,target:pick,amount:battle.stats(u).atk*scale});battle.healElements?.(u,pick,battle.stats(u).atk*scale,{ignoreHealingBlock:true});}return true;}
 ...
 if(profile.charId==='char_1019_siege2'&&skillIndex===0){u.siege2Next={skillCount:u.skillCount,scale:Number(bb.atkScale)||1,rangeId:profile.skill?.rangeId||null};suppressDefault=true;}
 ...
 if(u.id==='char_4145_ulpia'&&has(text,/若船锚停留的位置可以部署/)&&ctx.teleportActor){
  ...
  const dir=directionOf(u.dir||0),range=Math.max(1,Math.round(Number(bb.projectile_range)||1.8));
 ...
 if(has(text,/下次攻击|下一次攻击/)&&has(text,/(?:恢复|回复)/)&&Number.isFinite(config.healScale)){
  // 「下一次攻击」也要匹配（塞雷娅 S1 急救写的是「下一次攻击会为周围…恢复…」）。
 ...
 if(profile.charId==='char_4151_tinman'&&has(text,/炼金单元/)){
  ...
  const dir=directionOf(u.dir||0),reach=Math.max(1,Math.round(Number(bb.projectile_range)||1));
 ...
 if(profile.charId==='char_202_demkni'&&profile.skillIndex===2){ctx.addEffect(battle,{kind:'zone',sourceUid:u.uid,sourceDeployGen:u.deployGen,talentOrSkillId:'saria-s3',rangeUid:u.uid,x:u.x,y:u.y,radius:1,interval:1,nextAt:battle.s.time+1,endsAt:battle.s.time+(duration>0?duration:5),trackSide:'all',values:{sluggish:true,hot:battle.stats(u).atk*(Number(bb['attack@heal_scale'])||.2),fragile:Number(bb['demkni_s_3.damage_scale'])||1.4},snapshot:{},refKind:'owner',persistAfterSourceGone:false});}
...
export function onEvent(battle,type,payload,ctx){
 ...
 // 焰影苇草天赋「灼痕」：**造成伤害时 30% 概率**（黑板 `prob`）施加——攻击力-10%、15% 法术脆弱、6 秒、不可叠加。
 // 三件事是同一次触发，所以在这里专门处理，并在通用天赋循环里跳过她的「灼痕」（那里是无条件挂的）。
 // S3「生命火种」期间施加的灼痕持续时间变为无限，技能结束时清空这些灼痕（PRTS 备注）。
 if(type==='after-damage'&&source?.id==='char_1020_reed2'&&target&&source.kind!=='summon'&&payload.cause!=='dot'&&payload.cause!=='extra'){
  const talent=activeTalents(battle,source).find(t=>t.name==='灼痕'),tb=talent&&talentValues(talent);
  if(talent&&battle.economy.random()<(Number(tb.prob)||.3)){const idx=source.source?.skillIndex??battle.profile(source).skillIndex,s3=battle.skillActive(source)&&idx===2,duration=s3?1e9:(Number(tb.duration)||6);applyStatus(target,'burn',duration,{source:source.uid,resistible:false});const mark=(target.statuses||[]).find(x=>x.kind==='burn'&&x.source===source.uid);if(mark)mark.reed2Talent=true;const fragile=Number(tb.damage_scale)||1.15;applyStatus(target,'fragile',duration,{source:source.uid,value:fragile,resistible:false});const fmark=(target.statuses||[]).find(x=>x.kind==='fragile'&&x.source===source.uid);if(fmark)fmark.reed2Talent=true;target.fragile=Math.max(target.fragile||1,fragile);if(Number(tb.atk)<0)applyStatus(target,'attackDown',duration,{source:source.uid,value:Number(tb.atk),resistible:false});const amark=(target.statuses||[]).find(x=>x.kind==='attackDown'&&x.source===source.uid);if(amark)amark.reed2Talent=true;}
 }
 if(type==='skill-end'&&target?.id==='char_1020_reed2'){const idx=target.source?.skillIndex??battle.profile(target).skillIndex;if(idx===2)for(const e of battle.s.enemies){if(!e.statuses?.length)continue;const had=e.statuses.some(x=>x.reed2Talent);e.statuses=e.statuses.filter(x=>!x.reed2Talent);if(had){e.fragile=Math.max(1,...e.statuses.filter(x=>x.kind==='fragile').map(x=>Number(x.value)||1));}}}
 ...
 // 元素爆发（元素损伤条打满）的**全场**广播：凡是「有敌人爆发就触发」的天赋都挂在这里，不能按干员 id 过滤遍历对象。
 if(type==='element-burst'&&target){for(const u of battle.s.units.filter(v=>v.deployed&&v.hp>0)){
  if(u.id==='char_4146_nymph'){const talent=activeTalents(battle,u).find(t=>t.name==='窥心钥'),bb=talent&&talentValues(talent);if(talent)u.nymphStacks=Math.min(Number(bb.max_stack_cnt)||10,(u.nymphStacks||0)+1);}
  // 烛煌天赋「熔点引爆」：全场有**敌人**进入**灼燃损伤**爆发时，立即对其造成攻击力 250% 的**元素伤害**（PRTS 写「元素伤害」，
  // 所以走 `elementResistance` 的元素伤害通道，不是真实伤害）并回复 6% 最大生命；S3 弹药型技能期间另按技能黑板
  // 的 `ammo_recover` 补 2 发弹药。旧实现把遍历对象过滤成了「只遍历妮芙」，`u.id==='char_1040_blaze2'` 永远进不来；
  // 触发条件也必须按 PRTS 收窄到「敌人 + 灼燃」，别的元素爆发（凋亡／神经／腐蚀）不触发。
  if(u.id==='char_1040_blaze2'&&payload.element==='burn'&&battle.s.enemies.includes(target)){const t=activeTalents(battle,u).find(x=>x.name==='熔点引爆'),tb=t&&talentValues(t);if(tb){const event=payload.event;ctx.dealDamage(battle,{source:u,target,amount:battle.stats(u).atk*(Number(tb.ep_damage_scale)||2.5),type:'elemental',cause:'extra',parentEventId:event?.eventId,effectId:'blaze2-talent:'+u.uid+':'+(event?.eventId||0)});ctx.applyHeal(battle,{source:u,target:u,amount:u.maxHp*(Number(tb.hp_ratio)||.06)});if(battle.skillActive(u)&&(u.source?.skillIndex??battle.profile(u).skillIndex)===2){const recover=Number(skillBB(battle,u).ammo_recover)||2;u.ammo=Math.min(u.ammoMax??Infinity,(u.ammo||0)+recover);}}}
 }}
 ...
 if(type==='after-damage'&&source?.id==='char_4116_blkkgt'&&target&&source.blkkgtNext?.skillCount===source.skillCount){const next=source.blkkgtNext;source.blkkgtNext=null;const victims=zoneTargets(battle,source,next.rangeId).slice(0,next.max||1);for(const victim of victims.length?victims:[target])for(let n=0;n<next.hits;n++)ctx.dealDamage(battle,{source,target:victim,amount:battle.stats(source).atk*next.scale,type:'physical',cause:'extra',skill:true,effectId:'blkkgt-s1:'+source.uid+':'+victim.uid+':'+n});}
 ...
 // 妮芙（`char_4146_nymph`）的凋亡损伤/元素伤害，PRTS 干员页 68952 的备注逐条：
 //  * S1 笞心击/S2 怵然震爆/S3 心防溃决的攻击都附带凋亡损伤，量＝**该次伤害造成的生命值损失** × 黑板比例
 //    （PRTS：「计算凋亡损伤量时以目标受伤前后的生命值差的绝对值为准」）；
 //  * S1：目标处于凋亡爆发期间时**额外**造成攻击力 `attack@extra_ep_damage_scale`(30%) 的**元素伤害**；
 //  * S3：目标处于凋亡爆发期间时攻击造成**元素伤害**（伤害类型在 `native-battle.hit` 里改）；
 //  * 天赋「失魂」：攻击处于凋亡爆发期间的敌人时**标记**它，每秒受攻击力 40%（S2 命中时 70%）的元素伤害，直到爆发结束。
 if(type==='after-damage'&&source?.id==='char_4146_nymph'&&target&&source.kind!=='summon'&&payload.cause!=='extra'){
  const idx=source.source?.skillIndex??battle.profile(source).skillIndex,config=skillConfig(battle.profile(source)),bb=config.bb,bursting=elementBurstActive(target,'necrosis',battle.s.time),active=idx===0||battle.skillActive(source);
  if(active){const ratio=Number(bb['attack@ep_damage_ratio']??bb.ep_damage_ratio)||0,dealt=Math.max(0,Number(payload.result?.hp)||0);if(ratio>0&&dealt>0)ctx.applyElementDamage?.(battle,{source,target,amount:dealt*ratio,type:'necrosis',cause:'attack'});}
  if(idx===0&&bursting&&Number(bb['attack@extra_ep_damage_scale'])>0)ctx.dealDamage(battle,{source,target,amount:battle.stats(source).atk*Number(bb['attack@extra_ep_damage_scale']),type:'elemental',cause:'extra',parentEventId:payload.event?.eventId,effectId:'nymph-s1-burst:'+source.uid+':'+(payload.event?.eventId||0)});
  if(bursting){const talent=activeTalents(battle,source).find(t=>t.name==='失魂'),tb=talent&&talentValues(talent);if(talent)target.nymphSoul={sourceUid:source.uid,scale:Math.max(Number(tb.element_atk_scale)||.4,Number(bb.element_atk_scale)||0),until:target.elementBurstUntil};}
 }
 ...
 if(type==='after-damage'&&source?.id==='char_4039_horn'&&target&&source.kind!=='summon'&&(source.source?.skillIndex??battle.profile(source).skillIndex)===0){const bb=skillBB(battle,source);ctx.addEffect(battle,{kind:'zone',sourceUid:source.uid,sourceDeployGen:source.deployGen,talentOrSkillId:'horn-light-'+source.skillCount,x:target.x,y:target.y,radius:Number(bb.projectile_range)||1.7,interval:1,nextAt:battle.s.time+1,endsAt:battle.s.time+(Number(bb.projectile_delay_time)||6),trackArea:true,trackSide:'enemy',values:{reveal:true,shape:'circle'},snapshot:{},refKind:'owner',persistAfterSourceGone:false});}
 if(type==='after-damage'&&source?.id==='char_1040_blaze2'&&target&&source.kind!=='summon'&&battle.skillActive(source)&&(source.source?.skillIndex??battle.profile(source).skillIndex)===1){const bb=skillBB(battle,source);ctx.addEffect(battle,{kind:'zone',sourceUid:source.uid,sourceDeployGen:source.deployGen,talentOrSkillId:'blaze2-s2-'+target.uid,x:target.x,y:target.y,radius:1.5,interval:1,nextAt:battle.s.time+1,endsAt:battle.s.time+8,trackArea:true,trackSide:'enemy',values:{dot:true,sluggish:true,elementScale:Number(bb.element_damage_scale)||.3,elementOffDamage:true,type:'arts',elementType:'burn',atk_scale:Number(bb.atkScale)||.35},snapshot:{damage:battle.stats(source).atk*(Number(bb.atkScale)||.35)},refKind:'owner',persistAfterSourceGone:false});}
 // 烛煌「众恶的焚场」（S3，她的默认技能）：**只有目标处于灼燃损伤爆发期间**，攻击才额外造成攻击力 60% 的
 // 元素伤害（PRTS 文案「若目标处于灼燃损伤爆发期间则额外造成攻击力{attack@atk_scale}的元素伤害」）。
 // 这条原来被通用「攻击附带元素」兜底当成每次攻击都挂灼燃，于是她不开技也在累计灼燃（用户 2026-09-22 报）。
 // 注意：S3 是弹药型技能，她的攻击是普通攻击，所以判定挂在通用 after-damage（不是 payload.skill 分支）。
 if(type==='after-damage'&&source?.id==='char_1040_blaze2'&&target&&source.kind!=='summon'&&payload.cause!=='extra'&&battle.skillActive(source)&&(source.source?.skillIndex??battle.profile(source).skillIndex)===2){
  const scale=Number(skillBB(battle,source)['attack@atk_scale'])||0;
  if(scale>0&&elementBurstActive(target,'burn',battle.s.time))ctx.dealDamage(battle,{source,target,amount:battle.stats(source).atk*scale,type:'elemental',cause:'extra',parentEventId:payload.event?.eventId,effectId:'blaze2-s3-burst:'+source.uid+':'+(payload.event?.eventId||0)});
 }
 ...
 if(source.id==='char_332_archet'&&source.archetNext?.skillCount===source.skillCount&&payload.cause!=='extra'){const next=source.archetNext;source.archetNext=null;const splash=battle.s.enemies.filter(e=>e.uid!==target.uid&&e.hp>0&&!e.hidden&&Math.max(Math.abs(e.x-target.x),Math.abs(e.y-target.y))<=1).slice(0,next.max);for(const e of splash)ctx.dealDamage(battle,{source,target:e,amount:battle.stats(source).atk*next.splashScale,type:'physical',cause:'extra',skill:true,parentEventId:payload.event?.eventId,effectId:'archet-splash:'+source.uid+':'+(payload.event?.eventId||0)+':'+e.uid});}
   ...
   // 通用「攻击附带元素损伤」兜底只对**核对过的技能**启用（GENERIC_ELEMENT_SKILLS），数值只认原表显式的元素比例键。
   // 为什么不再「技能文案里出现 灼燃／凋亡／元素伤害 就按 atkScale 给每次攻击挂元素」：
   // 那会把「元素只写在区域／条件／受击分句里」的技能变成默认附属元素——2026-09-22 用户报
   // 「烛煌的攻击不知道为什么能累计灼烧」就是它（她的 S2 元素属于灼烧地段、S3 的元素只在目标处于灼燃爆发期间才有；
   // 妮芙 S3、菲莱 S2、塑心 S2 也是同一类分句）。同类已全部交给专属实现，不要再放开这条兜底：
   // 烛煌 S1/S2（灼燃领域与灼烧地段）、烛煌 S3（条件式元素伤害）、妮芙 S1/S2/S3、菲莱 S2（受击反伤）、塑心 S1/S2。
 ...
 if((type==='before-damage'||type==='after-damage')&&source&&source.kind!=='summon'&&target&&payload.cause!=='dot'&&payload.cause!=='reflect'){
  for(const talent of activeTalents(battle,source)){
   const text=talent.description||'',bb=talentValues(talent);
   if(source.id==='char_1020_reed2'&&has(text,/灼痕/))continue;   // 灼痕由专属块按 30% 概率处理
 ...
 if(type==='after-heal'&&source&&target){
  if(source.id==='char_1016_agoat2'&&target.uid!==source.uid){const talent=activeTalents(battle,source).find(t=>t.name==='氤氲'),tb=talent&&talentValues(talent);if(talent){const s3=battle.skillActive(source)&&(source.source?.skillIndex??battle.profile(source).skillIndex)===2,mult=s3?(Number(skillBB(battle,source).talent_scale)||1):1,amount=battle.stats(source).atk*(Number(tb.healScale)||.1)*mult;ctx.addEffect(battle,{kind:'hot',sourceUid:source.uid,sourceDeployGen:source.deployGen,targetUid:target.uid,talentOrSkillId:'agoat2-hot:'+target.uid,interval:1,nextAt:battle.s.time+1,endsAt:battle.s.time+(Number(tb.duration)||6),values:{heal:amount,elementRegen:amount},snapshot:{heal:amount},refKind:'owner',persistAfterSourceGone:false});}}
...
export function periodicMods(battle,u,ctx){
 ...
 if(u.id==='char_172_svrash')for(const e of battle.s.enemies.filter(e=>e.hp>0&&!e.hidden&&battle.inside(u,e,true)))ctx.revealEnemy(battle,e);
 if(u.id==='char_4087_ines')for(const e of battle.s.enemies.filter(e=>e.hp>0&&!e.hidden&&battle.inside(u,e,true))){ctx.revealEnemy(battle,e);applyStatus(e,'sluggish',1,{source:u.uid,resistible:false});}
 ...
 // 塑心天赋「无词哀歌」：**攻击范围内**敌人每秒受到攻击力 10% 的凋亡损伤并停顿 0.2 秒（PRTS 写「每秒」，
 // 不是「每次攻击」——所以她的天赋不再走通用天赋元素通道，见 TALENT_ELEMENT_DEDICATED 的排除）。
 if(u.id==='char_245_cello'&&battle.s.time>=(u.celloTalentNextAt||0)){u.celloTalentNextAt=battle.s.time+1;const t=activeTalents(battle,u).find(x=>x.name==='无词哀歌'),tb=t&&talentValues(t);if(t){const ratio=Number(tb.ep_damage_ratio)||.1,slow=Number(tb.sluggish)||0;for(const e of battle.s.enemies.filter(e=>e.hp>0&&!e.hidden&&battle.inside(u,e,true))){ctx.applyElementDamage?.(battle,{source:u,target:e,amount:battle.stats(u).atk*ratio,type:'necrosis',cause:'talent'});if(slow>0)applyStatus(e,'sluggish',slow,{source:u.uid,resistible:false});}}}
 if(u.id==='char_2026_yu'&&battle.s.time>=(u.yuTalentNextAt||0)){u.yuTalentNextAt=battle.s.time+1;const t=activeTalents(battle,u).find(x=>x.name==='礼尚往来'),tb=t&&talentValues(t);if(t){const interval=Number(tb.interval)||1,scale=Number(tb['yu_t_1[enemy].atk_scale'])||.4,ep=Number(tb['yu_t_1[enemy].ep_damage_ratio'])||.12;for(const e of battle.s.enemies.filter(e=>e.hp>0&&e.block===u.uid)){ctx.dealDamage(battle,{source:u,target:e,amount:battle.stats(u).atk*scale,type:'arts',cause:'talent'});ctx.applyElementDamage?.(battle,{source:u,target:e,amount:battle.stats(u).atk*ep,type:'burn',cause:'talent'});void interval;}}}
