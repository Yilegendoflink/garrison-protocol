...
import {initEnemyTraits,refreshEnemyTraitStats,tickEnemyTraits,enemyTraitAfterDamage,enemyTraitBeforeAttack,enemyTraitOnHit,enemyTraitAfterAttack,enemyTraitOnDeath,enemyNearbyExit,enemyStealAmmo,syncEnemyConcealMarker,applyEnemyTraitAuras} from './native-enemy-traits.js';
...
export class NativeBattle {
 ...
 path(route,flying){
  ...
  refreshEnemyAuras(){
   ...
   for(const blade of blades)if(guards.some(guard=>Math.hypot(guard.x-blade.x,guard.y-blade.y)<=1.4))blade.attackIntervalMod=-1.3;
  ...
  resolveEnemyStrike(enemy,target,action={}){if(enemyStealAmmo(this,enemy,target))return;const baseAtk=enemy.atk,baseType=enemy.damageType,hasDagger=Boolean(enemy.canAttack&&enemy.daggers>0),dagger=hasDagger?1+(Number(enemy.daggerAtkAdd)||0):1,rush=enemy.id==='enemy_1072_dlancer'&&enemy.dlancerArmed?(Number(enemy.dlancerStrikeSpeed||enemy.speed)||0)*600:0;let scale=Number(action.special?.scale??action.scale)||1;if(enemy.block!==target.uid&&Number(enemy.enemyTalent?.['Attack.attack@ranged_atk_scale'])>0)scale*=Number(enemy.enemyTalent['Attack.attack@ranged_atk_scale']);if(enemy.specialSkill?.prefab==='InvisibleCombat'){if((enemy.invisibleStrikeReady??enemy.invisible)&&enemy.block===target.uid&&!permissions(enemy).silenced){scale*=enemy.specialAtkScale||1;enemy.invisibleStrikeReady=false;}enemy.formInvisible=false;enemy.invisible=false;enemy.invisibleCombatWasActive=false;enemy.invisibleRecoverAt=this.s.time+6;}if(rush)enemy.dlancerArmed=false;enemy.atk=baseAtk*scale*dagger;if(action.special?.type||action.type)enemy.damageType=action.special?.type||action.type;this.hurt(target,enemy,{attackId:action.attackId,parentEventId:action.parentEventId});if(rush){enemy.atk=rush;enemy.damageType='physical';this.hurt(target,enemy);}enemy.atk=enemy.deathGrowthStacks!=null?enemy.baseAtk*(1+enemy.deathGrowthStacks*Number(enemy.enemyTalent['Attack.atk'])):baseAtk;enemy.damageType=baseType;if(rush){enemy.dlancerSpeedStacks=0;enemy.dlancerMoveTimer=0;enemy.speed=enemy.baseSpeed;}if(hasDagger){const used=Math.max(1,Math.floor(Number(enemy.daggerPerAttack)||1));enemy.daggers=Math.max(0,enemy.daggers-used);enemy.daggersUsed=(enemy.daggersUsed||0)+used;}if(!action.suppressAttackZone&&enemy.attackZone&&(!enemy.enemySkills?.some(s=>s.prefab==='PollutedRangedAtk')||action.special?.polluted))this.addEnemyGroundZone(enemy,enemy.attackZone,{x:target.x,y:target.y,follow:!!enemy.attackZone.follow,attackId:action.attackId??null});}
