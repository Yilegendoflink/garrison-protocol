...
import {equipmentStatMods} from './native-equipment.js';
...
export class NativeBattle {
 ...
 path(route,flying){
  ...
  refreshEnemyAuras(){for(const u of this.s.units)u.enemyAttackSpeedMod=0;for(const e of this.s.enemies)if(e.hp>0){e.def=e.baseDef??e.def;e.res=e.baseRes??e.res;e.damageResistance=0;e.attackSpeedMod=0;e.attackIntervalMod=0;}const active=this.s.enemies.filter(e=>e.hp>0&&!e.hidden&&e.aura);for(const source of active)for(const target of this.s.enemies)if(target!==source&&target.hp>0&&!target.hidden&&Math.max(Math.abs(source.x-target.x),Math.abs(source.y-target.y))<=source.aura.radius){target.def+=(source.aura.def||0);target.damageResistance=Math.max(target.damageResistance||0,source.aura.damageResistance||0);}const guards=this.s.enemies.filter(e=>e.hp>0&&!e.hidden&&/伙友卫队/.test(e.name||'')),blades=this.s.enemies.filter(e=>e.hp>0&&!e.hidden&&/伙友影刃/.test(e.name||''));for(const guard of guards)if(blades.some(blade=>Math.max(Math.abs(guard.x-blade.x),Math.abs(guard.y-blade.y))<=1.4))for(const u of this.s.units)if(u.deployed&&u.hp>0&&Math.max(Math.abs(guard.x-u.x),Math.abs(guard.y-u.y))<=1.1)u.enemyAttackSpeedMod=Math.min(u.enemyAttackSpeedMod||0,-30);for(const blade of blades)if(guards.some(guard=>Math.max(Math.abs(guard.x-blade.x),Math.abs(guard.y-blade.y))<=1.4))blade.attackIntervalMod=-1.3;}
  enemySpecialReady(enemy,target){const skill=enemy?.specialSkill;if(!skill||!target||skill.prefab==='stuncombat'||skill.prefab==='InvisibleCombat'||skill.prefab==='InvisibleShield'||skill.prefab==='Flame')return null;if(skill.prefab==='AOEAttack'&&enemy.firstAttackUsed)return null;if(skill.prefab==='CrossAttack'&&Math.abs(enemy.x-target.x)>1e-6&&Math.abs(enemy.y-target.y)>1e-6)return null;if(skill.prefab==='DeathEye'&&enemy.deathEye)return null;const first=enemy.firstAttackSplash&&!enemy.firstAttackUsed,countReady=skill.spCost>0&&(enemy.skillAttackCount||0)>=skill.spCost,cooldownReady=skill.spCost===0&&Number.isFinite(skill.cooldown)&&skill.cooldown>=0&&this.s.time>=(enemy.nextSkillAt??Infinity);if(!first&&!countReady&&!cooldownReady)return null;return {prefab:skill.prefab,scale:enemy.specialAtkScale||1,radius:Number(skill.bb?.range_radius)||1,splash:first||skill.prefab==='AOEAttack',stun:Number(skill.bb?.stun)||0,type:skill.prefab==='CrossAttack'?'arts':null,noDirectAttack:skill.prefab==='DeathEye',polluted:skill.prefab==='PollutedRangedAtk'};}
  resolveEnemyStrike(enemy,target,action={}){const baseAtk=enemy.atk,baseType=enemy.damageType,hasDagger=Boolean(enemy.canAttack&&enemy.daggers>0),dagger=hasDagger?1+(Number(enemy.daggerAtkAdd)||0):1,rush=enemy.id==='enemy_1072_dlancer'&&enemy.dlancerArmed?(Number(enemy.dlancerStrikeSpeed||enemy.speed)||0)*600:0;let scale=Number(action.special?.scale??action.scale)||1;if(enemy.invisible&&enemy.specialSkill?.prefab==='InvisibleCombat'){scale*=enemy.specialAtkScale||1;enemy.formInvisible=false;enemy.invisible=false;enemy.invisibleRecoverAt=this.s.time+6;}if(rush)enemy.dlancerArmed=false;enemy.atk=baseAtk*scale*dagger;if(action.special?.type)enemy.damageType=action.special.type;this.hurt(target,enemy);if(rush){enemy.atk=rush;enemy.damageType='physical';this.hurt(target,enemy);}enemy.atk=baseAtk;enemy.damageType=baseType;if(rush){enemy.dlancerSpeedStacks=0;enemy.dlancerMoveTimer=0;enemy.speed=enemy.baseSpeed;}if(hasDagger){const used=Math.max(1,Math.floor(Number(enemy.daggerPerAttack)||1));enemy.daggers=Math.max(0,enemy.daggers-used);enemy.daggersUsed=(enemy.daggersUsed||0)+used;}if(enemy.attackZone)this.addEnemyGroundZone(enemy,enemy.attackZone,{x:target.x,y:target.y,follow:!!enemy.attackZone.follow,attackId:action.attackId??null});}
  ...
  tickEnemyDeathEye(){for(const enemy of this.s.enemies){const channel=enemy.deathEye;if(!channel)continue;const target=getActor(this.s,channel.targetUid);if(enemy.hp<=0||!enemyTargetValid(target)){enemy.deathEye=null;enemy.stanceUntil=0;continue;}if(this.s.time<channel.endsAt){while(this.s.time+1e-9>=channel.nextAt){dealDamage(this,{source:enemy,target,amount:enemy.atk*channel.damageScale,type:'arts',cause:'skill'});channel.nextAt+=channel.interval;}continue;}for(const ally of attackableAllies(this.s))if(Math.max(Math.abs(ally.x-target.x),Math.abs(ally.y-target.y))<=channel.radius)applyElementDamage(this,{source:enemy,target:ally,amount:enemy.atk*channel.elementScale,type:'necrosis',cause:'skill'});this.emit('enemy-skill-end',{uid:enemy.uid,x:enemy.x,y:enemy.y,skill:'DeathEye',targetUid:target.uid});enemy.deathEye=null;enemy.stanceUntil=0;}}
  // 清明（enemy_1209_sfden）的 InvisibleShield：每 cooldown 秒给半径内的**其他**敌人上隐匿，自身不含。
  // 这是独立计时的自施法技能，不跟普通攻击绑定，所以放在每帧的敌人结算里，而不是走 enemySpecialReady
  ...
  tickEnemyInvisibleShield(){for(const enemy of this.s.enemies){const skill=enemy.specialSkill;if(!skill||skill.prefab!=='InvisibleShield'||enemy.hp<=0||enemy.hidden)continue;if(this.s.time<(enemy.nextSkillAt??Infinity))continue;const control=permissions(enemy);if(!control.skill||control.silenced)continue;const cooldown=Number(skill.cooldown)>0?Number(skill.cooldown):15,duration=Number(skill.bb?.duration)>0?Number(skill.bb.duration):3,radius=Number(skill.bb?.range_radius)>0?Number(skill.bb.range_radius):(Number(enemy.range)>0?Number(enemy.range):1.8);enemy.nextSkillAt=this.s.time+cooldown;let count=0;for(const other of this.s.enemies){if(other===enemy||other.hp<=0||other.hidden)continue;if(Math.max(Math.abs(other.x-enemy.x),Math.abs(other.y-enemy.y))>radius)continue;if(applyStatus(other,'invisible',duration,{source:enemy.uid,resistible:false}))count++;}this.emit('enemy-skill',{uid:enemy.uid,x:enemy.x,y:enemy.y,skill:'InvisibleShield',radius,duration,count});}}
  resolveEnemyAttackEffects(enemy,target,{count=true,extra=null}={}){if(!enemy||!target||target.hp<=0)return;if(count){enemy.attackCount=(enemy.attackCount||0)+1;enemy.skillAttackCount=(enemy.skillAttackCount||0)+1;}const skillAllowed=permissions(enemy).skill&&!permissions(enemy).silenced;if(skillAllowed&&extra?.stun>0)applyStatus(target,'stun',extra.stun,{source:enemy.uid,resistible:false});if(skillAllowed&&enemy.attackStunEvery>0&&enemy.attackStunDuration>0&&enemy.attackCount%enemy.attackStunEvery===0)applyStatus(target,'stun',enemy.attackStunDuration,{source:enemy.uid,resistible:false});if(enemy.attackElement&&enemy.attackElementScale>0)applyElementDamage(this,{source:enemy,target,amount:enemy.atk*enemy.attackElementScale,type:enemy.attackElement,cause:'attack'});if(enemy.bleeding&&skillAllowed)this.applyEnemyBleeding(enemy,target);}
  // 逐腐兽的流血：命中后周期性受到法术伤害，目标接受治疗时解除（治疗钩子在 heal 里）。
  applyEnemyBleeding(enemy,target){const spec=enemy.bleeding,trait=enemyBleedingTraitId(enemy);if(!spec||!target||target.hp<=0)return false;const interval=Math.max(.1,Number(spec.interval)||1);addEffect(this,{kind:'dot',sourceUid:enemy.uid,targetUid:target.uid,talentOrSkillId:trait,interval,nextAt:this.s.time+interval,endsAt:this.s.time+(Number(spec.duration)||10),values:{type:'arts',damage:Number(spec.damage)||0},snapshot:{damage:Number(spec.damage)||0},refKind:'owner',persistAfterSourceGone:true});this.emit('enemy-ability',{uid:enemy.uid,x:enemy.x,y:enemy.y,ability:'bleeding',targetUid:target.uid});return true;}
  ...
  resolveEnemyDeath(enemy,source=null){if(!enemy||enemy.enemyDeathHandled)return;enemy.enemyDeathHandled=true;const effect=enemy.deathExplosion;if(effect&&(!effect.requiresFire||enemy.onFire)){for(const target of attackableAllies(this.s).filter(u=>Math.max(Math.abs(u.x-enemy.x),Math.abs(u.y-enemy.y))<=effect.radius))dealDamage(this,{source:enemy,target,amount:enemy.atk*effect.scale,type:effect.type,cause:'extra',skipHooks:true});this.emit('enemy-ability',{uid:enemy.uid,x:enemy.x,y:enemy.y,ability:'death-explosion',radius:effect.radius,type:effect.type});}
 ...
 onEnemyDeath(enemy,info={}){
  ...
  this.deadSpawnFragments(enemy,info);
 }
 queueEnemySpawn(q,placement){this.pendingEnemySpawns??=[];this.pendingEnemySpawns.push({q,placement});}
 flushEnemySpawns(){const rows=this.pendingEnemySpawns;if(!rows?.length)return;this.pendingEnemySpawns=[];for(const row of rows)this.spawn(row.q,row.placement);}
 ...
 hurt(u,e){const p=this.profile(u),skillIndex=p.skillIndex??u.source?.skillIndex;if(u.id==="char_311_mudrok"&&u.invulnerableUntil>this.s.time)return;if(u.id==="char_1032_excu2"&&skillIndex===1&&this.skillActive(u)&&e.damageType==="physical"&&this.economy.random()<Number(blackboard(p.skill?.blackboard).prob||0)){u.ammo=Math.min(u.ammoMax||Infinity,(u.ammo||0)+Number(blackboard(p.skill?.blackboard).recover_cnt||1));return;}let evade=this.behavior(u).evasion,evadeProb=branchTrait(p).values.prob??evade;if(u.physicalEvadeOnce&&e.damageType==='physical'){u.physicalEvadeOnce=false;evade=1;evadeProb=1;}else if(u.physicalEvadeUntil>this.s.time&&['physical'].includes(e.damageType)){evade=1;evadeProb=u.physicalEvadeProb||0;}else if(u.skillEvasionProb>0&&this.skillActive(u)&&['physical','arts'].includes(e.damageType)){evade=1;evadeProb=u.skillEvasionProb;}const tippiTalent=(p.activeTalents||[]).find(t=>t.name==='片场工作指南'),tippiQuiet=tippiTalent&&this.s.time-(u.lastDamagedAt??u.deployAt??0)>=Number(blackboard(tippiTalent.blackboard).stack_time||9);if(tippiQuiet&&e.damageType!=='true'){u.lastDamagedAt=this.s.time;this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}if(u.id==='char_420_flamtl'&&u.flamFollowUp==null)u.flamFollowUp=false;const flamAura=this.s.units.some(v=>v.deployed&&v.hp>0&&v.id==='char_420_flamtl'&&(this.profile(v).activeTalents||[]).some(t=>t.name==='红松骑士团团长')&&Array.isArray(this.profile(u).bonds)&&this.profile(u).bonds.includes('kazimierzShip'));if(flamAura&&e.damageType!=='true'&&this.economy.random()<.22){if(u.id==='char_420_flamtl')u.flamFollowUp=true;this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}const mountainTalent=p.charId==='char_264_f12yin'&&(p.activeTalents||[]).find(t=>t.name==='强壮肉体'),mountainProb=mountainTalent?Number(blackboard(mountainTalent.blackboard).prob)||.15:0;if(mountainProb>0&&e.damageType==='physical'&&this.economy.random()<mountainProb){this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}const lapSkill=(p.charId==='char_140_whitew'&&(skillIndex??0)===0&&this.skillActive(u)),lapProb=lapSkill?Number(blackboard(p.skill.blackboard).prob):0;if(lapProb>0&&e.damageType==='physical'&&this.economy.random()<lapProb){this.s.effects.push({x:u.x,y:u.y,text:'抵挡',life:.5,type:'block'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'block'});return;}const armor=(p.activeTalents||[]).find(t=>t.name==='战术装甲'),armorProb=armor?Number(blackboard(armor.blackboard).prob):0;if(armorProb>0&&this.economy.random()<armorProb){this.s.effects.push({x:u.x,y:u.y,text:'抵挡',life:.5,type:'block'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'block'});return;}if(evade&&['physical','arts'].includes(e.damageType)&&this.economy.random()<evadeProb){if(u.id==='char_420_flamtl')u.flamFollowUp=true;this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}let value=damage({amount:e.atk,type:e.damageType,defense:this.stats(u).def,resistance:this.stats(u).magicResistance});if(this.on('emptyShip'))value*=.8;
  const enemyAttack=e.atk*(1+Math.min(0,statusAttributeChanges(e).attack||0));value=damage({amount:enemyAttack,type:e.damageType,defense:this.stats(u).def,resistance:this.stats(u).magicResistance});if(this.on('emptyShip'))value*=.8;if(u.damageResistance>0)value*=Math.max(0,1-u.damageResistance);const titi=this.s.units.find(v=>v.id==='char_4056_titi'&&v.deployed&&v.hp>0&&this.skillActive(v)&&(this.profile(v).skillIndex??v.source?.skillIndex)===2);if(titi&&u.id!==titi.id&&u.hp-value<=0){u.hp=1;applyStatus(u,'sleep',Math.max(1,titi.skillLeft||5),{source:titi.uid,resistible:false});return;}if(u.id==='char_4064_mlynar'&&this.s.enemies.filter(x=>x.hp>0&&Math.max(Math.abs(x.x-u.x),Math.abs(x.y-u.y))<=1).length>=3)value*=.85;const rmixerIndex=p.skillIndex??u.source?.skillIndex,rmixerBB=rmixerIndex===1?blackboard(p.skill?.blackboard):null;if(p.charId==='char_4194_rmixer'&&this.skillActive(u)&&rmixerIndex===1&&rmixerBB&&u.ammo>=Number(rmixerBB.ammo_cost||25)&&u.hp-value<=1){u.ammo-=Number(rmixerBB.ammo_cost||25);u.hp=1;this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'guard'});return;}dealDamage(this,{source:e,target:u,value,type:e.damageType,cause:'attack'});u.lastDamagedAt=this.s.time;if(spTypeOf(this.profile(u).skill)==='INCREASE_WHEN_TAKEN_DAMAGE')gainSp(u,this.profile(u).skill,undefined,this.spCost(u));
 }
  tickBondIdle(u,dt){
  ...
  for(const e of this.s.enemies){tickStatuses(e,dt);this.tickEnemyRevive(e);if(e.artsWeak?.until<this.s.time)e.artsWeak=null;if(e.hp>0&&e.regen>0)e.hp=Math.min(e.maxHp,e.hp+e.regen*dt);if(e.hp>0&&!e.lowHpTriggered&&e.lowHpRatio>0&&e.hp/e.maxHp<=e.lowHpRatio){e.lowHpTriggered=true;if(e.lowHpAttackMultiplier>0)e.atk=e.baseAtk*e.lowHpAttackMultiplier;if(e.lowHpMoveMultiplier>0)e.speed=e.baseSpeed*e.lowHpMoveMultiplier;if(e.lowHpUnblockTime>0){e.unblockable=true;e.unblockableUntil=this.s.time+e.lowHpUnblockTime;}this.emit('enemy-phase',{uid:e.uid,x:e.x,y:e.y,phase:'low-hp'});}if(e.unblockableUntil!=null&&this.s.time>=e.unblockableUntil)e.unblockable=false;}this.refreshEnemyAuras();this.tickEnemyDeathEye();this.tickEnemyInvisibleShield();this.flushEnemySpawns();
  ...
  for(const e of this.s.enemies){if(e.hp<=0||e.trainingDummy)continue;this.ensureEnemySelfField(e);const control=permissions(e),alive=attackableAllies(this.s);
   ...
   const target=e.hidden?null:e.block?alive.find(u=>u.uid===e.block&&enemyTargetValid(u)):e.ranged?alive.filter(u=>!u.invisible&&!permissions(u).sleeping&&(enemyTargetInRange(e,u)||(e.specialSkill?.prefab==='CrossAttack'&&(Math.abs(e.x-u.x)<=1e-6||Math.abs(e.y-u.y)<=1e-6)))).sort((a,b)=>compareEnemyTargets({tauntLevel:a.kind==='summon'?0:this.stats(a).tauntLevel,deployAt:a.deployAt||0,uid:a.uid},{tauntLevel:b.kind==='summon'?0:this.stats(b).tauntLevel,deployAt:b.deployAt||0,uid:b.uid}))[0]:null;e.attackCooldown=Math.max(0,e.attackCooldown-1);
   if(!control.attack||e.hidden)e.action=null;if(e.action&&--e.action.left<=0){const action=e.action,u=getActor(this.s,action.target);e.action=null;if(u&&u.hp>0){const special=action.special;this.emit('strike',{uid:e.uid,x:e.x,y:e.y,targetX:u.x,targetY:u.y,ranged:e.ranged,enemy:true,type:special?.type||e.damageType,style:special?.prefab||'single'});if(special?.prefab==='DeathEye'){this.startEnemyDeathEye(e,u);}else{this.resolveEnemyStrike(e,u,action);this.resolveEnemyAttackEffects(e,u,{extra:special});if(special?.splash)for(const extra of alive)if(extra.uid!==u.uid&&Math.max(Math.abs(extra.x-e.x),Math.abs(extra.y-e.y))<=special.radius){this.resolveEnemyStrike(e,extra,{scale:special.scale,type:special.type});this.resolveEnemyAttackEffects(e,extra,{count:false,extra:special});}if(special?.polluted){this.emit('enemy-skill',{uid:e.uid,x:u.x,y:u.y,skill:'PollutedRangedAtk',targetUid:u.uid});}}if(special){if(e.specialSkill?.spCost>0)e.skillAttackCount=0;e.nextSkillAt=this.s.time+(Number(e.specialSkill?.cooldown)>0?Number(e.specialSkill.cooldown):Infinity);e.firstAttackUsed=true;}e.lastAttackAt=this.s.time;if(e.movementPolicy===ENEMY_MOVEMENT_POLICIES.BURST_THEN_MOVE&&e.burstShots>0){e.burstFired=(e.burstFired||0)+1;if(e.burstFired>=e.burstShots){e.burstFired=0;e.burstUntil=this.s.time+(e.burstCooldown||0);}}}}
   const special=control.skill&&!control.silenced?this.enemySpecialReady(e,target):null,specialOnly=Boolean(e.specialSkill?.prefab==='CrossAttack'&&e.range<=0),meleeScale=e.block===target?.uid&&e.meleeAttackScale>0?e.meleeAttackScale:1,preparedSpecial=special&&meleeScale!==1?{...special,scale:special.scale*meleeScale}:special;if(target&&e.canAttack&&control.attack&&!e.action&&!e.attackCooldown&&!Number(e.burstUntil)&&(!specialOnly||special)){if(e.movementPolicy===ENEMY_MOVEMENT_POLICIES.BURST_THEN_MOVE&&e.burstTarget!==target.uid){e.burstTarget=target.uid;e.burstFired=0;}const t=attackTiming(Math.max(.1,e.interval+(e.attackIntervalMod||0)),Math.max(10,Math.min(600,e.attackSpeed+(e.attackSpeedMod||0))),windupSeconds(Math.max(.1,e.interval+(e.attackIntervalMod||0))));e.attackCooldown=t.frames;e.action={left:t.windupFrames,target:target.uid,special:preparedSpecial,scale:meleeScale,attackId:newAttackId(this)};}
   const hold=e.formHold||enemyShouldHoldPosition(e,{target:specialOnly&&!special?null:target,now:this.s.time}),beforeX=e.x,beforeY=e.y,beforeCmd=e.cmd,beforeHidden=e.hidden;let escaped=advanceEnemy(e,dt,kind=>this.emit(kind,{uid:e.uid,x:e.x,y:e.y}),hold);const progressed=e.cmd!==beforeCmd||Math.hypot(e.x-beforeX,e.y-beforeY)>1e-7||e.hidden!==beforeHidden;if(e.id==='enemy_1072_dlancer'){const moving=progressed&&!hold&&!e.block&&permissions(e).move&&e.route?.[beforeCmd]?.kind==='move';if(moving){e.dlancerMoveTimer+=dt;while(e.dlancerMoveTimer>=.5&&e.dlancerSpeedStacks<25){e.dlancerMoveTimer-=.5;e.dlancerSpeedStacks++;}e.speed=e.baseSpeed*Math.pow(1.5,e.dlancerSpeedStacks);}else if(!e.block){e.dlancerMoveTimer=0;e.dlancerSpeedStacks=0;e.speed=e.baseSpeed;}}if(progressed){e.lastProgressAt=this.s.time;e.stallTime=0;}else if(!hold&&!e.block&&e.speed>0&&permissions(e).move&&e.route?.[e.cmd]?.kind==='move'){e.stallTime=(e.stallTime||0)+dt;if(e.stallTime>=(e.stallTimeout||2)){e.action=null;e.stanceUntil=0;e.burstUntil=0;e.stallTime=0;this.emit('enemy-recover',{uid:e.uid,x:e.x,y:e.y,reason:'movement-stall'});escaped=advanceEnemy(e,dt,kind=>this.emit(kind,{uid:e.uid,x:e.x,y:e.y}),false);}}
