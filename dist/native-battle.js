...
import {operatorRegistry,attackModifier,attackPenetration,coinCapFor,coinGainAtSkillStart,grantCoins,spendCoins} from './native-operator-effects.js';
...
export class NativeBattle {
 constructor(data,economy,map,turn,{restore=false}={}){
  ...
  if(restore)return;
  this.s={frame:0,time:0,cost:20,costInitial:20,costMin:0,costMax:99,costRecoveryInterval:1,costRecoveryClock:0,units:[],enemies:[],projectiles:[],queue:[],damage:{},leaks:0,kills:0,finished:false,benchmark:!!turn.isBossTurn,limit:turn.isBossTurn?turn.bossTurnHpReduceTime:turn.normalPhaseTime,effects:[],events:[],strikes:[],banner:null,nextId:100000};
 ...
 updateBranch(u,dt){
  ...
  if(p.branch==='merchant'){
   const interval=Math.max(.1,Number(trait.interval)||3),fee=Math.abs(Number(trait.cost)||0);
   ...
   while(fee>0&&u.deployed&&u.hp>0&&this.s.time+1e-9>=u.merchantNextFeeAt){
    if(!this.spendCost(fee)){commitExit(this,{target:u,reason:'merchant-fee'});break;}
 ...
 spendCost(amount,{allowDebt=false}={}){const value=Number(amount);if(!Number.isFinite(value)||value<=0)return false;const next=this.s.cost-value;if(next<this.s.costMin)return false;if(!allowDebt&&next<0)return false;this.s.cost=next;this.emit('cost-spend',{amount:value,total:this.s.cost});return true;}
 ...
 tickCost(dt){const base=this.s.costRecoveryInterval;if(!Number.isFinite(base)||base<=0||this.s.cost>=this.s.costMax)return 0;const interval=base/Math.max(.01,this.costRecoveryMultiplier());this.s.costRecoveryClock+=Math.max(0,Number(dt)||0);let gained=0;while(this.s.costRecoveryClock+1e-9>=interval&&this.s.cost<this.s.costMax){this.s.costRecoveryClock-=interval;gained+=this.gainCost(1);}return gained;}
 ...
 step(){
  if(this.s.finished)return;if(this.s.settle.fault)throw Error('战斗结算异常');if(!this.s.settle.queue.length){this.s.settle.byId={};this.s.settle.consumed=[];}const dt=1/FPS;this.s.frame++;this.s.time=this.s.frame/FPS;this.tickCost(dt);while(this.s.queue.length&&this.s.queue[0].at<=this.s.time)this.spawn(this.s.queue.shift());
