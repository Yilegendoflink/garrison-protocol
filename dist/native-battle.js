...
import {settleEgirSwallow,tickDoll,enemyOpponents,enemyWineBuffs,ensureBattleShape,migrateBattle,validateBattle,dealDamage,applyHeal,applyRegen,applyLoss,applyElementDamage,addEffect,commitExit,reviveActor,tickLogic,effectStatMods,dispatch,newAttackId,attackableAllies,getActor,blockingActors,alliedActors,operatorSkillConfig,moveActor,teleportActor,canRelocateTo,nearbySpots,spawnSummon,grantGuard,chebyshev} from './native-effects.js';
...
export class NativeBattle {
 ...
 rangeWithSkill(u,skill=false,forceSkill=false){const p=this.profile(u),useSkill=(skill||forceSkill)&&p.skill?.rangeId,r=u.dollForm?this.data.ranges[branchTrait(p).rangeId]:useSkill?this.data.ranges[p.skill.rangeId]:p.range;let grids=r?.grids||[{row:0,col:1}];if(p.branch==='fortress'&&!grids.some(g=>g.row===0&&g.col===0))grids=grids.concat({row:0,col:0});return {skill:useSkill,rangeId:useSkill?p.skill.rangeId:p.rangeId,cells:grids.map(g=>{let x=g.col,y=-g.row;for(let i=0;i<u.dir;i++)[x,y]=[-y,x];return{x:u.x+x,y:u.y+y};})};}
