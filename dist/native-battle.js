...
import {scheduleWaveQueue} from './native-wave-random.js';
...
export class NativeBattle {
 constructor(data,economy,map,turn,{restore=false}={}){
  ...
  if(restore)return;
  this.s={elementRulesVersion:1,frame:0,time:0,cost:20,costInitial:20,costMin:0,costMax:99,costRecoveryInterval:1,costRecoveryClock:0,enemyCostRecoveryMultiplier:1,enemyRespawnTimeMultiplier:1,mlyssFirstRhineDiscountUsed:false,bondLateranoAmmoStacks:0,bondEgirReviveCount:0,bondYanGuardiansSpawned:false,units:[],enemies:[],projectiles:[],queue:[],damage:{},leaks:0,kills:0,derivedKills:0,finished:false,benchmark:!!turn.isBossTurn,limit:turn.isBossTurn?turn.bossTurnHpReduceTime:turn.normalPhaseTime,effects:[],events:[],strikes:[],banner:null,nextId:100000};
  ...
  for(const u of this.s.units){const stats=this.stats(u);u.maxHp=stats.maxHp;if(!u.deployed&&u.exitLife==null)u.hp=stats.maxHp;}
  settleEgirSwallow(this);
 }
 ...
 enemyHealthChanged(enemy){checkWEnrage(this,enemy);checkZaroCageHealth(this,enemy);enemyFormHealthChanged(this,enemy);}
 ...
 enemyDamageReceived(enemy,opts,result){checkWEnrage(this,enemy);checkZaroCageHealth(this,enemy);enemyFormAfterDamage(this,enemy,result);enemyTraitAfterDamage(this,enemy,opts,result);if(result.total>0&&opts.cause!=='dot'&&opts.cause!=='loss'){enemySpEvent(enemy,'INCREASE_WHEN_TAKEN_DAMAGE');enemyTraitOnDamageSp(this,enemy);}}
 ...
 behavior(u){
  ...
  if(u.dollForm)return {...base,attack:u.id!=='char_1023_ghost2',damageType:u.id==='char_4016_kazema'?'arts':base.damageType,antiAir:false};
  ...
  if(!target?.deployed||target.neutral||target.hp<=0||target.isolated||target.downed||target.healable===false||(target.unhealable&&source?.uid!==target.uid))return false;
 ...
 releaseNativeAttack(u,action){
  ...
  if(kind==='heal'){
   ...
   }else for(const id of action.targets){const target=this.s.units.find(v=>v.uid===id);if(!target)continue;this.heal(u,target,action.amount);if(behavior.elementHealing)this.healElements(u,target,action.amount*(trait.ep_heal_ratio??.5)*(target.haroldElementScale??1));target.haroldElementScale=1;}
 ...
 params(id){return bondBlackboard(this.data,id);}
 ...
 stats(u){
  ...
  if(has('yanShip')){const b=this.params('yanShip'),v=bondLayerValue(b,'base_atk','atk_per_stack',l.yanShip);ratio('atk',v,'盟约·炎');}
  if(has('egirShip')){const b=this.params('egirShip'),v=bondLayerValue(b,'base_max_hp','max_hp_per_stack',l.egirShip);ratio('maxHp',v,'盟约·阿戈尔');}
  if(this.on('steadShip')){const b=this.params('steadShip'),v=bondLayerValue(b,'base_max_hp','max_hp_per_stack',l.steadShip);ratio('maxHp',v,'盟约·坚守');}
  if(this.on('deputShip')){const b=this.params('deputShip'),v=bondLayerValue(b,'base_def','def_per_stack',l.deputShip);ratio('def',v,'盟约·助力');base.respawnTime*=Math.max(0,1+bondValue(b,'respawn_time',-.3));}
  const preciBB=this.params('preciShip');if(this.on('preciShip')&&(has('preciShip')||(this.rows.preciShip.count>=bondValue(preciBB,'power_bond_char_cnt',3)&&p.position==='RANGED'))){const v=bondLayerValue(preciBB,'base_atk','atk_per_stack',l.preciShip);ratio('atk',v,'盟约·精准');}
  if(has('soloShip')){const b=this.params('soloShip');ratio('atk',bondValue(b,'atk',.6),'盟约·独行');ratio('maxHp',bondValue(b,'max_hp',.6),'盟约·独行');}
  if(this.on('suntShip')&&p.isGolden)ratio('atk',bondValue(this.params('suntShip'),'power_atk',.3),'盟约·绝技');
  if(this.on('raidShip')){const b=this.params('raidShip'),v=bondValue(b,'power_attack_speed',50);if((l.raidShip||0)>=bondValue(b,'power_bond_stack_cnt',50)){as+=v;note('attackSpeed','add',v,'盟约·突袭');}}
  ...
  if(this.on('skillfulShip')){const b=this.params('skillfulShip'),radius=(l.skillfulShip||0)>=bondValue(b,'power_bond_stack_cnt',40)?1.42:1.01;if(this.s.units.some(v=>v.deployed&&v.hp>0&&this.owns(v,'skillfulShip')&&Math.hypot(u.x-v.x,u.y-v.y)<radius)){const v=bondLayerValue(b,'base_attack_speed','attack_speed_per_stack',l.skillfulShip);as+=v;note('attackSpeed','add',v,'盟约·灵巧');}}
  if(this.on('sargonShip')&&this.owns(u,'sargonShip')){u.sargonBuffs=(u.sargonBuffs||[]).filter(x=>x.endsAt>this.s.time);const b=this.params('sargonShip'),count=Math.min(bondValue(b,'max_buff_stack_cnt',25),u.sargonBuffs.length);as+=count*bondValue(b,'base_attack_speed',12);if(this.rows.sargonShip.count>=bondValue(b,'power_bond_char_cnt',6))ratio('atk',count*(Number(b.base_atk)||.12),'盟约·萨尔贡');}
  if(this.on('lateranoShip')&&this.owns(u,'lateranoShip')&&this.rows.lateranoShip.count>=bondValue(this.params('lateranoShip'),'power_bond_char_cnt',6)){const b=this.params('lateranoShip');ratio('atk',Math.min(bondValue(b,'max_atk_for_consume',2),(this.s.bondLateranoAmmoStacks||0)*bondValue(b,'atk_per_consume',.04)),'盟约·拉特兰');}
  // 吞噬攻击力在最终加算层结算，不参与技能/装备乘算。

  if(u.raidBuffUntil>this.s.time){const b=this.params('raidShip');ratio('atk',bondLayerValue(b,'base_atk','atk_per_stack',l.raidShip,.25,.01),'盟约·突袭');ratio('maxHp',bondLayerValue(b,'base_max_hp','max_hp_per_stack',l.raidShip,.25,.01),'盟约·突袭');}if(u.garrisonDeployBuff&&this.s.time<u.garrisonDeployBuff.endsAt){base.atk+=u.garrisonDeployBuff.atk||0;base.maxHp+=u.garrisonDeployBuff.maxHp||0;note('atk','add',u.garrisonDeployBuff.atk||0,'部署卫戍');note('maxHp','add',u.garrisonDeployBuff.maxHp||0,'部署卫戍');}
  ...
  for(const item of u.source.equipment){const record=this.data.season.trapChessDataDict[item.chessId];for(const effect of this.data.season.effectBuffInfoDataDict[record?.effectId]||[]){const b=blackboard(effect.blackboard);if(effect.key==='char_attribute_mul'){mul('atk',b.atk??1,'装备');mul('maxHp',b.max_hp??1,'装备');mul('def',b.def??1,'装备');}else if(effect.key.startsWith('env_gbuff')&&!equipGenericExcluded(b.key)){ratio('atk',b.atk||0,'装备');ratio('maxHp',b.max_hp||0,'装备');ratio('def',b.def||0,'装备');as+=b.attack_speed||0;base.magicResistance+=b.magic_resistance||0;base.spRecoveryPerSec+=b.sp_recovery_per_sec||0;}}}equipmentStatMods(this,u,{ratio,note,base,addAttackSpeed:value=>{as+=value;}});u.sargonEquipmentShares=(u.sargonEquipmentShares||[]).filter(x=>x.endsAt>this.s.time);for(const share of u.sargonEquipmentShares){const owner=this.s.units.find(v=>v.uid===share.ownerUid);if(owner)equipmentStatMods(this,owner,{ratio,note,base,addAttackSpeed:value=>{as+=value;}});}
  if(has('victoriaShip')&&u.source.equipment.length){const b=this.params('victoriaShip');if(this.rows.victoriaShip.count>=bondValue(b,'power_bond_char_cnt',6)){const normal=u.source.equipment.filter(i=>!this.data.season.trapChessDataDict[i.chessId].isGolden).length,golden=u.source.equipment.length-normal,base=bondValue(b,'atk_normal_equip',.5),extra=bondValue(b,'atk_golden_equip',.3);ratio('atk',normal*base+golden*(base+extra),'盟约·维多利亚');}}
 ...
 garrisonBonds(u,bb){
  ...
  if(type==='bond_actived_maxstack'){const rows=this.economy.bonds();const ids=Object.keys(rows).filter(id=>rows[id].active);if(!ids.length)return[];const max=Math.max(...ids.map(id=>this.layers[id]||0));return[this.economy.pick(ids.filter(id=>(this.layers[id]||0)===max))];}
 ...
 // 该单位当前生效的 rangeId：本帧挂起的技能范围 > 常驻的被动范围 > 开技中的技能范围 > 常态范围。
 //  * 瞬时／AUTO 技能（duration 0、-1）在开技那一帧由 activate() 写 `skillRangeHold`，否则
 //    「下一击／立即」类技能会因为 skillLeft=0 而永远按常态范围判定（塞雷娅 S1 只能治自身格就是这来的）。
 //  * PASSIVE 技能与文案写「被动效果：攻击范围扩大」的技能（引星棘刺 S3）范围常驻生效。
 skillRangeId(u,skill=false,forceSkill=false){
  const p=this.profile(u);
  if(u.dollForm)return branchTrait(p).rangeId;
  const held=u.skillRangeHoldAt===this.s.time&&u.skillRangeHold&&this.data.ranges[u.skillRangeHold]?u.skillRangeHold:null;
  if(held)return held;
  const passive=p.skill?.rangeId&&(p.skill.skillType==='PASSIVE'||/被动效果/.test(String(p.skill.description||'')))?p.skill.rangeId:null;
  if(passive&&this.data.ranges[passive])return passive;
  if((skill||forceSkill)&&p.skill?.rangeId)return p.skill.rangeId;
  return p.rangeId;
 }
 // 把 range_table 的 (col,row) 网格按朝向旋转并落到棋盘坐标。方向约定 0=右、1=下、2=左、3=上。
 cellsForGrids(u,grids){return (grids||[]).map(g=>{let x=g.col,y=-g.row;for(let i=0;i<(u.dir||0);i++)[x,y]=[-y,x];return{x:u.x+x,y:u.y+y};});}
 ...
 deadSpawnFragments(enemy,{reason}={}){
  ...
  for(let i=0;i<count;i++){
   ...
   this.queueEnemySpawn({id:spec.enemyKey,derived:true},{x:spot.x,y:spot.y,route:enemy.route,cmd},this.economy.random()*.7);
  }
...
u.skillRangeHold=sk.rangeId||null;u.skillRangeHoldAt=this.s.time;const skillAir=skillAntiAir(p.charId,p.skillIndex??u.source?.skillIndex);u.skillAir=skillAir==null?null:{value:skillAir,count:u.skillCount,until:this.s.time+(Number(sk.duration)>0?Number(sk.duration):0)};const lateranoBonus=this.on('lateranoShip')&&this.owns(u,'lateranoShip')?this.params('lateranoShip'):null,baseAmmo=ammoCount(sk),bondAmmo=lateranoBonus?Math.max(0,Math.floor(baseAmmo*(Number(lateranoBonus.base_ammo_percent||0)+Number(lateranoBonus.ammo_percent_per_stack||0)*(this.layers.lateranoShip||0)))):0;u.ammo=kind==='ammo'?baseAmmo+cfg.ammoBonus+(u.talentAmmoBonus||0)+bondAmmo+(p.charId==='char_1032_excu2'?Math.min(4,this.s.units.filter(v=>v.deployed&&v.hp>0&&this.profile(v)?.bonds?.includes('lateranoShip')).length):0):0;u.ammoMax=u.ammo;u.ammoPerAttack=cfg.ammoPerAttack;u.skillLeft=kind==='ammo'?0:this.skillTimeLeft(sk);if(kind==='instant'){const t=attackTiming(this.stats(u).baseAttackTime,this.stats(u).attackSpeed,windupSeconds(this.stats(u).baseAttackTime,p.attackWindup));u.spLock=t.seconds;}this.event(u,'skill');this.emit('skill-start',{uid:u.uid,kind,name:sk.name,x:u.x,y:u.y,wide:this.wideAttack(u),wideKind:this.wideKind(u)});if(dispatch(this,'skill-start',{target:u}))return;if(kind==='instant'&&sk.skillType==='AUTO'&&spTypeOf(sk)==='INCREASE_WHEN_ATTACK'){u.enhanced=true;return;}if(kind==='instant'){const targets=this.targets(u);if(p.branch!=='incantationmedic'&&/回复.*生命|治疗/.test(sk.description||'')){for(const v of this.healingTargets(u))this.heal(u,v,this.stats(u).atk*(cfg.bb.healScale??b.heal_scale??b.atk_scale??1));}else if(cfg.atkScale!=null||b.atk_scale){for(const e of targets.slice(0,cfg.multiTarget===Infinity?targets.length:(cfg.multiTarget??b.max_target??999)))for(let hit=0;hit<Math.max(1,cfg.hits||1);hit++)this.hit(u,e,this.stats(u).atk*(cfg.atkScale??b.atk_scale??1),this.baseDamageType(u),{skill:true});}if(b.stun||cfg.bb.stun)for(const e of targets){if(applyStatus(e,'stun',b.stun??cfg.bb.stun,{source:u.uid}))this.emit('control',{uid:e.uid,kind:'stun',x:e.x,y:e.y});}}}
 deactivate(u){const p=this.profile(u),sk=p.skill;if(!sk||!this.skillActive(u))return false;const idx=sk.skillIndex??u.source?.skillIndex;if(u.id==='char_1033_swire2'&&idx===2){const cfg=operatorSkillConfig(this,u),coins=Math.max(0,Math.trunc(u.coins||0));for(let i=0;i<coins;i++){const targets=this.targets(u);if(!targets.length)break;const target=targets[Math.floor(this.economy.random()*targets.length)];this.hit(u,target,this.stats(u).atk*(cfg.atkScale||1),'physical');moveActor(this,target,u,sk.description||'');}u.coins=0;}if(u.id==='char_4039_horn'&&idx===1&&u.ammo>0){const cfg=operatorSkillConfig(this,u),targets=this.targets(u);for(let i=0;i<u.ammo;i++)for(const target of targets)this.hit(u,target,this.stats(u).atk*(Number(cfg.bb['attack@s2.atk_scale'])||1.6),'physical',{skill:true});u.hp=Math.max(1,u.hp-u.maxHp*(Number(cfg.bb['attack@s2.hp_ratio'])||.6));u.ammo=0;}u.skillLeft=0;u.ammo=0;u.action=null;this.emit('skill-end',{uid:u.uid,x:u.x,y:u.y});dispatch(this,'skill-end',{target:u});return true;}
  spCost(u){const owner=u?.kind==='summon'?this.s.units.find(v=>v.uid===u.ownerUid):null,p=this.profile(u),base=p.skill?.spData.spCost||0,bb=this.on('suntShip')?this.params('suntShip'):null;return bb&&this.rows.suntShip.count>=bondValue(bb,'ex_char_cnt',5)&&(p.isGolden||owner&&this.profile(owner).isGolden)?Math.floor(base*bondValue(bb,'sp_ratio',.7)):base;}
 gainCost(amount){const value=Number(amount);if(!Number.isFinite(value)||value<=0)return 0;const before=this.s.cost;this.s.cost=Math.min(this.s.costMax,before+value);const gained=this.s.cost-before;if(gained>0)this.emit('cost-gain',{amount:gained,total:this.s.cost});return gained;}
  ...
  hit(u,e,amount,type,opts={}){if(e.hp<=0||e.hidden||e.invulnerable||permissions(e).sleeping)return;amount*=yinYangAttackScale(u,e);const skill=!!opts.skill||this.skillActive(u),p=this.profile(u),pen=attackPenetration(this,u,e),enemyStatus=statusAttributeChanges(e);const preciPen=this.on('preciShip')&&(this.owns(u,'preciShip')||p.position==='RANGED')&&this.rows.preciShip.count>=bondValue(this.params('preciShip'),'power_bond_char_cnt',3)?this.params('preciShip'):null,equipPen=Math.max(pen.ratio,equipMagicPenetration(this,u));let penetrationRatio=Math.max(preciPen?bondValue(preciPen,'power_def_penetrate',.3):0,equipPen),resistancePenetration=Math.max(preciPen?bondValue(preciPen,'power_magic_resist_penetrate',.3):0,equipPen);const physical=damage({amount,type:'physical',defense:Math.max(0,e.def+(enemyStatus.defense||0)-pen.fixed),penetrationRatio}),arts=damage({amount,type:'arts',resistance:Math.max(0,e.res+(enemyStatus.resistance||0)+(enemyStatus.magicResistance||0)-Number(pen.magicFixed||0)),penetrationRatio:resistancePenetration}),elemental=damage({amount,type:'elemental',elementResistance:Number(e.elementResistance)||0});if(p.charId==='char_1014_nearl2'&&(p.skillIndex??u.source?.skillIndex)===2&&type!=='true'&&(e.block===u.uid||this.s.summons.some(x=>x.type==='nearl2-sun'&&x.ownerUid===u.uid&&e.block===x.uid)))type='true';if(p.charId==='char_4146_nymph'&&(p.skillIndex??u.source?.skillIndex)===2&&this.skillActive(u)&&elementBurstActive(e,'necrosis',this.s.time))type='elemental';if(type!=='true'&&(this.s.band==='band_chen'||p.garrisons.some(g=>blackboard(g.blackboard).key==='act1autochess_gar_eff_chaos')||equipWeakness(this,u)))type=physical>=arts?'physical':'arts';let value=type==='physical'?physical:type==='arts'?arts:type==='elemental'?elemental:amount;
  ...
  if(!result)return;
  if(!opts.bondExtra&&this.on('kazimierzShip')&&this.rows.kazimierzShip.count>=bondValue(this.params('kazimierzShip'),'power_bond_char_cnt',6)&&this.owns(u,'kazimierzShip')&&!this.s.enemies.some(x=>x.hp>0&&x.block===u.uid))dealDamage(this,{source:null,target:e,amount:this.stats(u).atk*(Number(this.params('kazimierzShip').pure_atk_scale)||.3),type:'true',cause:'extra',skipHooks:true});
 ...
 hurt(u,e,{attackId=null,parentEventId=null,sourceLess=false,damageAmount=null,cause='attack',defPenetration=null}={}){if(u.neutral){const value=damage({amount:Number.isFinite(damageAmount)?damageAmount:this.enemyAttackDamage(e,1,u),type:e.damageType,defense:(u.def||0)*(1-(defPenetration??e.enemyDefPenetration??0)),resistance:u.res||0});return dealDamage(this,{source:sourceLess?null:e,target:u,value,type:e.damageType,cause,attackId,parentEventId});}const p=this.profile(u),skillIndex=p.skillIndex??u.source?.skillIndex;if(u.id==="char_311_mudrok"&&u.invulnerableUntil>this.s.time)return;if(u.id==="char_1032_excu2"&&skillIndex===1&&this.skillActive(u)&&e.damageType==="physical"&&this.economy.random()<Number(blackboard(p.skill?.blackboard).prob||0)){u.ammo=Math.min(u.ammoMax||Infinity,(u.ammo||0)+Number(blackboard(p.skill?.blackboard).recover_cnt||1));return;}let evade=this.behavior(u).evasion,evadeProb=branchTrait(p).values.prob??evade;if(u.physicalEvadeOnce&&e.damageType==='physical'){u.physicalEvadeOnce=false;evade=1;evadeProb=1;}else if(u.physicalEvadeUntil>this.s.time&&['physical'].includes(e.damageType)){evade=1;evadeProb=u.physicalEvadeProb||0;}else if(u.skillEvasionProb>0&&this.skillActive(u)&&['physical','arts'].includes(e.damageType)){evade=1;evadeProb=u.skillEvasionProb;}const tippiTalent=(p.activeTalents||[]).find(t=>t.name==='片场工作指南'),tippiQuiet=tippiTalent&&this.s.time-(u.lastDamagedAt??u.deployAt??0)>=Number(blackboard(tippiTalent.blackboard).stack_time||9);if(tippiQuiet&&e.damageType!=='true'){u.lastDamagedAt=this.s.time;this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}if(u.id==='char_420_flamtl'&&u.flamFollowUp==null)u.flamFollowUp=false;const flamAura=this.s.units.some(v=>v.deployed&&v.hp>0&&v.id==='char_420_flamtl'&&(this.profile(v).activeTalents||[]).some(t=>t.name==='红松骑士团团长')&&Array.isArray(this.profile(u).bonds)&&this.profile(u).bonds.includes('kazimierzShip'));if(flamAura&&e.damageType!=='true'&&this.economy.random()<.22){if(u.id==='char_420_flamtl')u.flamFollowUp=true;this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}const mountainTalent=p.charId==='char_264_f12yin'&&(p.activeTalents||[]).find(t=>t.name==='强壮肉体'),mountainProb=mountainTalent?Number(blackboard(mountainTalent.blackboard).prob)||.15:0;if(mountainProb>0&&e.damageType==='physical'&&this.economy.random()<mountainProb){this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}const lapSkill=(p.charId==='char_140_whitew'&&(skillIndex??0)===0&&this.skillActive(u)),lapProb=lapSkill?Number(blackboard(p.skill.blackboard).prob):0;if(lapProb>0&&e.damageType==='physical'&&this.economy.random()<lapProb){this.s.effects.push({x:u.x,y:u.y,text:'抵挡',life:.5,type:'block'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'block'});return;}const armor=(p.activeTalents||[]).find(t=>t.name==='战术装甲'),armorProb=armor?Number(blackboard(armor.blackboard).prob):0;if(armorProb>0&&this.economy.random()<armorProb){this.s.effects.push({x:u.x,y:u.y,text:'抵挡',life:.5,type:'block'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'block'});return;}if(evade&&['physical','arts'].includes(e.damageType)&&this.economy.random()<evadeProb){if(u.id==='char_420_flamtl')u.flamFollowUp=true;this.s.effects.push({x:u.x,y:u.y,text:'闪避',life:.5,type:'evade'});this.emit('hit',{uid:u.uid,x:u.x,y:u.y,type:'evade'});return;}let value=damage({amount:e.atk,type:e.damageType,defense:this.stats(u).def,resistance:this.stats(u).magicResistance});if(this.on('emptyShip'))value*=.8;
  ...
  tickBondIdle(u,dt){
   ...
   const lastAction=Math.max(Number(u.lastAttack??u.deployAt??0),Number(u.lastSkill??-Infinity));
  ...
  for(const u of this.s.units){
   ...
   if(!u.dollForm)tickTimeSp(u,skill,dt,stats.spRecoveryPerSec??1,{requiresBlock:!!behavior.spRequiresBlock,blocking,cost});
  ...
  if(this.s.benchmark){if(this.s.time>=this.s.limit)this.finish('timeout');}else if((!this.s.queue.some(q=>this.isPrimaryEnemy(q.id))&&!this.s.enemies.some(e=>!e.nonPrimary)&&!this.s.pendingEnemySpawns?.some(row=>this.isPrimaryEnemy(row.q.id)))||this.s.time>=this.s.limit||Math.min(ROUND_LEAK_CAP,this.s.leaks)>=this.economy.s.hp)this.finish('complete');
 }
