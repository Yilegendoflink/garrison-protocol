...
import {operatorRegistry,attackModifier,attackPenetration} from './native-operator-effects.js';
import {ensureBattleShape,migrateBattle,validateBattle,dealDamage,applyHeal,applyRegen,applyLoss,commitExit,reviveActor,tickLogic,effectStatMods,dispatch,newAttackId,attackableAllies,getActor,blockingActors,alliedActors,operatorSkillConfig} from './native-effects.js';
...
export class NativeBattle {
 ...
 hit(u,e,amount,type){if(e.hp<=0||e.hidden||e.invulnerable||permissions(e).sleeping)return;const p=this.profile(u),pen=attackPenetration(this,u,e),enemyStatus=statusAttributeChanges(e);let penetrationRatio=this.on('preciShip')&&this.rows.preciShip.count>=3&&(this.owns(u,'preciShip')||p.position==='RANGED')?.3:0;penetrationRatio=Math.max(penetrationRatio,pen.ratio);const physical=damage({amount,type:'physical',defense:Math.max(0,e.def+(enemyStatus.defense||0)-pen.fixed),penetrationRatio}),arts=damage({amount,type:'arts',resistance:Math.max(0,e.res+(enemyStatus.resistance||0)+(enemyStatus.magicResistance||0)),penetrationRatio});if(type!=='true'&&(this.s.band==='band_chen'||p.garrisons.some(g=>blackboard(g.blackboard).key==='act1autochess_gar_eff_chaos')))type=physical>=arts?'physical':'arts';let value=type==='physical'?physical:type==='arts'?arts:amount;
  ...
  if(!result)return;
  if(u.pendingAttackHeal&&!u.pendingAttackHeal.used){const ally=alliedActors(this.s).filter(v=>v.deployed&&v.hp>0&&this.canHeal(v,u)&&Math.hypot(v.x-u.x,v.y-u.y)<=1.5&&(v.uid!==u.uid||u.hp<u.maxHp)).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp||a.uid-b.uid)[0];if(ally){this.heal(u,ally,this.stats(u).atk*u.pendingAttackHeal.scale);u.pendingAttackHeal.used=true;u.pendingAttackHeal=null;}}
