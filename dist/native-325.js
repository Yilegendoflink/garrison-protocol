...
// https://github.com/itorr/homo (MIT). Display-only; combat values stay numeric.

export const homo = ((Nums) => {
...
export function format325(n){
 ...
 const key=Object.is(n,-0)?'-0':String(n);
...
export function rewrite325Text(str){
 return String(str).replace(/(?<![.\d])(-?(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?)(?![.\d])/g,(m,token)=>{
...
const SKIP=new Set(['INPUT','TEXTAREA','SCRIPT','STYLE']);
export function apply325Display(root){
 if(!root)return;
 const walk=node=>{
  if(!node)return;
  if(node.nodeType===1){
  ...
  if(node.nodeType===3){
   if(node._egg325)return;
   const next=rewrite325Text(node.nodeValue);
   if(next!==node.nodeValue){node.nodeValue=next;node._egg325=true;}
  }
 };
 walk(root);
}
