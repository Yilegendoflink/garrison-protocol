...
// 指令来源是两型集团军指挥员；作为交互依赖编入，不增补随机池或地图波次。
for(const id of ['enemy_10125_uacomd','enemy_10125_uacomd_2']){const entry=base.enemies[id]?.levels.find(x=>x.level===0);if(entry)data.enemyDependencies[id]=buildEnemyProfile(id,entry.data,catalogById[id]);}
