...
| 初雪 | 每当范围内1名敌人进入冻结时，有50%概率使已激活的【谢拉格】层数+1 ／ …+2 | 作战能力（战斗中） | `native-battle.js` `tickGarrisonStatusEvents`→`garrisonStatusEvent`（`check_ab_flag=16`、读 `prob`、按持有者各结算一次） | ✅／回归 `tests/native-kjerag-freeze.test.mjs` |
...
| 凛御银灰 | 使身前一格【谢拉格】干员获得特质“范围内1名敌人进入冻结时，有60%概率使已激活的【谢拉格】层数+1” ／ 使身前一格【谢拉格】干员获得特质“范围内1名敌人进入冻结时，有60%概率使已激活的【谢拉格】层数+2” | 作战能力（战斗中） | `native-effects.js` `applyGarrisonTransfers`（`give_garrison_to_front` + `check_bond_id=kjeragShip` 门禁）→ 目标拿到 `garrison_29_a/b`，冻结时按上面的状态事件结算（a 版每场 200 层上限） | ✅／回归 `tests/native-kjerag-freeze.test.mjs` |
...
| 银灰 | 每当范围内1名敌人进入冻结时，有50%概率使已激活的【谢拉格】层数+1 ／ …+2 | 作战能力（战斗中） | 同初雪（两人各持一条 `garrison_28`，一次冻结各结算一次） | ✅／回归 `tests/native-kjerag-freeze.test.mjs` |
...
- ~~敌人进入冻结（初雪、银灰，以及凛御银灰转发出去的 `garrison_29`）~~：**已补**——`native-battle.tickGarrisonStatusEvents` 每帧末尾比对新增状态，`garrisonStatusEvent` 认 `act1autochess_gar_event_enemy_abflag_inrange`（`check_ab_flag=16`、`prob`、按持有者各结算一次）；回归 `tests/native-kjerag-freeze.test.mjs`。
...
~~凛御银灰「使身前一格【谢拉格】干员获得特质…」不校验 `check_bond_id`，会把特质给任意身前一格~~：**已修**——`applyGarrisonTransfers` 的 `ok()` 用 `battle.owns(v,bond)` 过滤，非谢拉格的前一格不发（回归 `tests/native-kjerag-freeze.test.mjs`）。
...
5. ~~**`check_bond_id` 的校验强度**：凛御银灰身前一格不是谢拉格时，是不发还是照发？~~ **已定：不发**（`battle.owns(v,bond)` 过滤，回归 `tests/native-kjerag-freeze.test.mjs`）。
...
| 富文本内容 | 原表尖括号里除了样式标签（`<@ba.vup>`／`<$ba.stun>`／`</>`，丢掉）还有**内容**标签（`<铜灯盘>`／`<替身>`／`<寻呼模块>`／`<炎>`／`<盐坨子炮>`，原表还有 `<在场<@autochess.dgreen>6</>名不同【炎】干员>` 这种嵌套），此前一律被 `replace(/<[^>]+>/g,'')` 删掉。新增 `protocol.richText`（按尖括号配对扫描，内容标签保留内部文字），展示路径全部改走它：`native-play` 的 `plain`、`native-skill-text` 的 `plainText`、`native-wave-editor` 的敌人描述、`build-native` 烘进 `enemyIndex` 的 `desc`；解析逻辑仍保留原来的去标签写法 | `protocol.js`、`native-play.js`、`native-skill-text.js`、`native-wave-editor.js`、`scripts/build-native.mjs` |
...
外加 `tests/native-rich-text.test.mjs`（5 例）：样式标签丢掉、内容标签保留（含嵌套）、技能/天赋里的 `<替身>`、敌人图鉴里的 `<盐坨子炮>`，以及全量 2000+ 条展示文本「不残留尖括号、内容标签一个都不丢」。
